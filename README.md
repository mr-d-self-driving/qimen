...
这轮更新的重点是**把问事从“一次性出整块 JSON”推进到“边算边出、出错能自愈、模型可按环境切换、文案不臆造也不漏指标”**。

- **奇门 / 八字问事流式输出**：问事结果不再等模型一次性返回完整 JSON 再上屏，而是用哨兵分段（`<<<SEC:...>>>`）协议把每个面向用户的散文段独立流式推送，配合前端能量球加载与定格动效逐段补丁到对应卡片槽位。规则引擎产物先行上屏，AI 断语随后逐段补齐。
- **结构校验重试**：后端对流式结果做结构性硬失败判定——空流、核心段缺失、`data_json` 不可解析时，会发 `llm_retry` 事件让前端清空半截内容、重置回骨架，并在后端用非流式方式重试一次；再失败才降级报错。个别可选字段缺失走静默兜底，不误触发重试。
- **title 独立散文段**：奇门与八字问事都把标题（`summary.title` / `m1_conclusion.title`）拆成独立流式段，点明问题类型与核心结论，不再混在结构化 JSON 里重复输出。
- **问事模型可按环境切换**：新增 `QUESTION_MODEL` 环境变量，让问事模型可逐环境配置（代码默认回退 `gemini-3.1-pro-preview`），目前 production 与 preview 均切到 `gemini-3-flash-preview` 做效果验证，对比 flash 与 pro 的稳定性与质量。
- **反臆造与化气格约束**：在奇门与八字提示词中加硬约束——只能引用后端实际给出的盘面 / 四柱字段，严禁脑补盘面不存在的门星神、宫位、生克刑冲合害，或原局不存在的地支自刑、三合；化气 / 成象遇到“疑似 / 假化 / 受损”或恋印不化时按扶抑格局解读，不颠倒喜用忌神方向。
- **禁止泄漏后端计算指标**：面向用户的任何文案都不得出现“强度74.4”“能量值80”“置信度0.8”等内部量化数值，必须改用命理语言定性表达（如“身强足以担财”“引动有力”“气势偏弱”）。

### 上一轮更新（2026-06-06）

这轮更新的重点是**把八字问事从“模型会不会说得像”推进到“规则是否能被评测、页面是否同源展示、历史记录是否能安全回放”**。

- **用神 / 目标十神评测纠偏**：新增 [`docs/eval/yongshen-eval-2026-06.md`](./docs/eval/yongshen-eval-2026-06.md) 与 [`scripts/eval-yongshen.mjs`](./scripts/eval-yongshen.mjs)，用陆致极《八字命理学进阶教程》命例对照本地规则引擎，逐案标注吻合、部分、偏差，并记录从儿格、调候为急、印星救主、弃官就印等修复原因。
- **双轴语义路由**：八字问事不再只用旧的 `status/timing/pattern/character` 单轴分类，而是拆成 `framework` 与 `target_source`。前者决定走当前状态、应期扫描、先天结构、人物画像或开放战略；后者决定目标来自后端十神、命局用神还是低置信模型推断。
- **八字动态 panel 拾取逻辑**：动态面板从 `state_report`、`dynamic_report`、`timing_candidates` 拾取目标十神、宫位和岁运引动，避免页面只复述模型长文。高置信链路要求前端呈现与规则引擎同源，低置信链路则明确降级。
- **历史记录兼容**：旧 `analysis_mode` 会迁移到新双轴语义，存量问事记录继续回放；新记录按更稳定的字段契约展示，减少旧字段、空 panel、旧报告渲染断层。
- **关于页更新**：应用内“关于”页新增本轮更新与历史更新说明，面向用户解释为什么要做古籍命例评测、动态 panel 和双轴路由。
