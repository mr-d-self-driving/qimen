...
在 `src/views/HomeView.vue` 与 `src/views/BaziView.vue` 顶部填入你的 Supabase 项目 URL 和 Anon Key，完成。
...
│   └── fortune-daily.js      # 日运推演接口：历法计算 · JS 算分 · LLM 文案 · 缓存管理
│
├── lib/
...
│   ├── router/index.js       # 路由配置（奇门首页 / 八字 / 我的）
│   ├── store.js              # 全局状态（抽屉开关等）
│   ├── styles/global.css     # 全局 CSS 变量 · 背景 · 字体
│   └── views/
