...
function buildSpecialPatternContext(ctx = {}) {
  ...
  if (candidate?.subtype) {
    ...
    if (candidate.override_scope) lines.push(`覆盖范围：${candidate.override_scope}`);
    if (candidate.treatment) lines.push(`取用策略：${candidate.treatment}`);
    if (Array.isArray(candidate.affected_dimensions) && candidate.affected_dimensions.length) {
      lines.push(`影响字段：${candidate.affected_dimensions.join('、')}`);
    }
