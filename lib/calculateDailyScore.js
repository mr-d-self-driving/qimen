...
/**
 * 检查某五行对应的十神是否属于喜用/忌仇，返回合化得分
 * 逻辑：只要列表中有一个十神为喜用 → +5；全为忌仇 → -5；否则 → +1
 *
 * @param {string[]} shiShenList  十神名称列表
 * @param {string[]} favorable   喜用神数组
 * @param {string[]} unfavorable 忌仇神数组
 * @returns {number}
 */
function calcHeHuaScore(shiShenList, favorable, unfavorable) {
  const hasFav  = shiShenList.some(s => favorable.includes(s));
  const hasUnfav = shiShenList.some(s => unfavorable.includes(s));
...
function calculateDailyScore(profile, lunarDay) {
  ...
  // 喜用 / 忌仇神列表（快捷引用）
  // ⚠️ 兼容两种格式：
  //   - 新格式：数组 ["印星", "比劫"]（bazi.js 修改后落库的格式）
  //   - 旧格式：逗号字符串 "印星, 比劫"（旧数据的格式）
  ...
  let dim1Score = 0;
  if (favorable.includes(shiShen)) {
    // 属喜用神 → +8
    dim1Score = 8;
  } else if (unfavorable.includes(shiShen)) {
    // 属忌仇神 → -8
    dim1Score = -8;
  }
  // 闲神 → 0，保持默认值
  ...
  if (hasSanXing(liuZhi, yuanZhisArr)) {
    ...
    if (liuHeInfo) {
      ...
      if (hePartner === yuanRiZhi || hePartner === yuanNianZhi) {
        ...
        dim2Score += calcHeHuaScore(heShiShenList, favorable, unfavorable);
      }
    }

    // 三合判断（流日地支与原局地支形成三合半/三合）
    // 只要流日地支与原局中任意一支同在同一三合局，即触发合化加减分
    ...
    if (sanHeInfo) {
      ...
      if (hasSanHePartner) {
        // 三合半也给予合化加减分（力量略小，但使用同一档）
        const sanHeShiShenList = wuxingToShiShenList(sanHeHuaWuxing, riGan);
        dim2Score += calcHeHuaScore(sanHeShiShenList, favorable, unfavorable);
      }
    }
  }
