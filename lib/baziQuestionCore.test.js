...
test('buildBaziQuestionPrompt forwards upstream image analysis through status static and timing pipelines', () => {
  ...
  for (const route of routes) {
    ...
    assert.match(result.prompt, /形象判断：从财格，匹配度86%（成象），已覆盖常规喜忌。/);
  }
});
...
test('liunian传参：resolveCandidateYears所有time_scope枚举值在liunianList为空时均返回空years', () => {
  ...
  const scopes = [
    ...
    { type: 'specified_dayun', dayun_ganzhi: '辛巳', start_year: 2029, end_year: 2038 },
  ]

  for (const scope of scopes) {
    const result = resolveCandidateYears({ ...baseArgs, timeScope: scope })
    assert.deepEqual(result.years, [],
      `time_scope.type=${scope.type} 应返回空 years，但得到 ${JSON.stringify(result.years)}`)
    assert.ok(result.limitations.some(l => l.includes('流年')),
      `time_scope.type=${scope.type} 应有流年不足的 limitation，但得到 ${JSON.stringify(result.limitations)}`)
  }
})
