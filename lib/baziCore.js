...
const BAZI_ENGINE_VERSION = '1.8.27';
...
function applyClassicalStrengthOverlay(strengthResult, classicalPattern) {
    if (!_hasClassicalSignal(classicalPattern, '金木间隔') || !_hasClassicalSignal(classicalPattern, '自旺')) {
        return strengthResult;
    }
    if (strengthResult.strongWeak === '身强') return strengthResult;
    ...
    const strengthDetail = strengthResult.strengthDetail
        ...
        : strengthResult.strengthDetail;
    const overlayText = '金木间隔古法：乙坐卯自旺，两庚间隔成器，source-backed 校准为身强。';

    return {
        ...strengthResult,
        strongWeak: '身强',
        strengthBand: '偏强',
        totalStrengthScore: Math.max(Number(strengthResult.totalStrengthScore || 0), 5.8),
        strengthDetail,
        strengthBasis: `${strengthResult.strengthBasis || ''} ${overlayText}`.trim()
    };
}
