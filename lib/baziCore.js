...
const GE_JU_INFO = {
    ...
    '正财格': {
        ...
        chengGeTypes: ['财旺生官格', '化伤为财生官格', '财旺身弱用印格', '财格佩印格', '财带暗官格', '合杀存财格', '弃杀就财格', '财喜食生格', '用财喜印格', '用财喜比格'],
...
function getChengGe(baziInfo) {
    ...
    if (geju === '七杀格') {
        ...
        if (states.shi.touGan && states.shi.hasRoot && states.sha.hasRoot) return setResult('食', '杀邀食制格', '成格', '食神有根，制伏七杀。', '杀', states.shi.stem);
        ...
        if (states.shi.touGan && states.sha.hasRoot && !seal.hasQi) return setResult('食', '杀邀食制格', '成格', '食神透出而无印，制伏七杀。', '杀', states.shi.stem);
    ...
    if (geju === '正财格' || geju === '偏财格') {
        ...
        if (yongState.hasLi && monthHiddenGans.includes(states.shang.stem) && states.guan.hasQi) {
            return setResult(yongTenGod, '化伤为财生官格', '成格', '化伤为财，财旺生官。', '官', yongState.stem);
        }
        ...
        if (yongState.hasLi && states.sha.hasRoot && !states.sha.touGan && monthHiddenGans.includes(states.sha.stem)) {
            return setResult(yongTenGod, '弃杀就财格', '成格', '财透杀藏，弃杀就财。', '杀', yongState.stem);
        }
    ...
    if (geju === '伤官格') {
        ...
        // 金水伤官喜官：调候清贵，官透即喜
        if (isMetalWater && states.guan.touGan && !states.sha.touGan) return setResult('官', '伤官喜官格', '成格', '金水日主，伤官见官反成清贵，官为用。', '伤', states.guan.stem);
        // 伤官带杀，用伤制杀
        if (states.sha.touGan && states.sha.hasRoot && !seal.hasLi) return setResult('伤', '伤官带杀格', '成格', '伤官当令而制伏七杀。', '杀', states.shang.stem);
        // 身弱伤官佩印：印为用，化伤生身护格
        if (dayStrength === '身弱' && seal.hasLi) return setResult(states.yin.touGan ? '印' : '枭', '伤官佩印格', '成格', '身弱伤官泄重，佩印生身护格。', '伤', sealStem);
        // 化伤为财生官（财官皆显）
        if (finance.touGan && states.guan.hasQi && !states.sha.touGan) return setResult(caiUse, '化伤为财生官格', '成格', '伤官化财，财旺生官，财为通关。', '官', caiStem);
        // 伤官生财：财透得生，财为用
        if (finance.touGan) return setResult(caiUse, '伤官生财格', '成格', '伤官吐秀而财星得生。', '伤', caiStem);
        // 印强能化（非身弱亦可佩印）
        if (seal.hasLi) return setResult(states.yin.touGan ? '印' : '枭', '伤官佩印格', '成格', '伤官外露而印绶能化。', '伤', sealStem);
    ...
    if (geju === '建禄格') {
        ...
        if (states.guan.touGan && states.guan.hasRoot && !states.shang.touGan && !states.sha.touGan) return setResult('官', '建禄用官格', '成格', '建禄身旺，官透有根，以官为用、印护官为喜。', '印', states.guan.stem);
...
const BaziEngine = {
    ...
    getGeJu: function (bazi) {
        ...
        return {
            ...
            monthMergeInfo,
            note: monthMergeInfo
                ? `${monthZhi}参与${monthMergeInfo.mergeType}化${monthMergeInfo.resultElement}，合化保留为候选；主格仍以${monthZhi}本气${monthMainGan}立格。`
...
function buildCompleteBaziDetail({ baZi, yun, isMale = true, currentYear = new Date().getFullYear() }) {
    ...
    const mapTransitCycle = (dy, index) => {
        ...
        const liunianList = dy.getLiuNian().map(ln => {
            ...
            const liuyueList = ln.getLiuYue().map((ly, monthIndex) => {
                const lyGanZhi = ly.getGanZhi() || '';
                return buildTransitItem(lyGanZhi, {
                    index: monthIndex,
