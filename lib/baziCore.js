...
const BAZI_ENGINE_VERSION = '1.8.28';
...
function getChengGe(baziInfo) {
    ...
    const resolveQiongTongPattern = () => {
        ...
        if (dayGan === '丁' && monthZhi === '辰') {
            const hasExposedWaterKilling = hasStem('壬') || hasStem('癸');
            const hasWaterFrame = ['申', '子', '辰'].every(branch => hasBranch(branch));
            if (hasStem('庚') && !hasExposedWaterKilling && !hasWaterFrame) {
            ...
            if (hasStem('戊') && (hasExposedWaterKilling || hasWaterFrame)) {
    ...
    if (geju === '正财格' || geju === '偏财格') {
        ...
        if (states.shi.touGan && states.shi.hasRoot && yongState.touGan && yongState.hasRoot && !official.touGan) {
            return setResult('食', '财喜食生格', '成格', '财星透干有根，食神亦透而有根，食神生财为用。', yongTenGod, states.shi.stem, {
                protectedShens: ['食', '财', '才'],
                promotedShens: ['食'],
                assistantShens: ['财', '才'],
                demotedShens: ['官', '杀'],
    ...
    if (geju === '食神格') {
        ...
        if (
            ...
            return setResult('食', '食神生财格', '成格', '癸水卯月食神当令，日主身强而财星透出，食神生财为用。', '财', states.shi.stem, {
                protectedShens: ['食', '财', '才'],
                promotedShens: ['食'],
                assistantShens: ['财', '才'],
                demotedShens: ['印', '枭'],
    ...
    if (geju === '建禄格') {
        ...
        if (
            ...
            return setResult('伤', '建禄伤官生财格', '成格', '壬水亥月身旺气寒，伤官透出吐秀，财星有气承接，取伤官生财为用。', '财', states.shang.stem, {
                protectedShens: ['伤', '财', '才'],
                promotedShens: ['伤'],
                assistantShens: ['财', '才'],
                demotedShens: ['印', '枭'],
                methodTags: ['身旺气寒', '伤官生财', '扶弱用神']
            });
        }
...
function _hasClassicalSignal(classicalPattern, signal) {
    if (!classicalPattern) return false;
    return classicalPattern.name === signal
        || (classicalPattern.aliases || []).includes(signal)
        || (classicalPattern.methodTags || []).includes(signal);
}

function applyClassicalStrengthOverlay(strengthResult, classicalPattern) {
    const overlays = [
        {
            matched: _hasClassicalSignal(classicalPattern, '金木间隔') && _hasClassicalSignal(classicalPattern, '自旺'),
            score: 5.8,
            summary: '古法金木间隔成器，乙坐卯自旺，source-backed 校准为身强。',
            text: '金木间隔古法：乙坐卯自旺，两庚间隔成器，source-backed 校准为身强。'
        },
        {
            matched: _hasClassicalSignal(classicalPattern, '伤官吐秀') && _hasClassicalSignal(classicalPattern, '卯木为用'),
            score: 5.9,
            summary: '古法伤官吐秀，辰土生金拱水，source-backed 校准为身强。',
            text: '伤官吐秀古法：两辰拱水、两庚印透，辰土反生金助水，source-backed 校准为身强。'
        },
        {
            matched: _hasClassicalSignal(classicalPattern, '财旺生官') && _hasClassicalSignal(classicalPattern, '去火存金') && _hasClassicalSignal(classicalPattern, '体用不伤'),
            score: 5.8,
            summary: '古法财旺生官去火存金，癸亥时日主之气已贯，source-backed 校准为身强。',
            text: '财旺生官古法：癸亥时日主之气已贯，巳亥遥冲去火存金，体用不伤，source-backed 校准为身强。'
        }
    ];
    const overlay = overlays.find(item => item.matched);
    if (!overlay || strengthResult.strongWeak === '身强') return strengthResult;

    const strengthDetail = strengthResult.strengthDetail
        ? {
            ...strengthResult.strengthDetail,
            band: '偏强',
            summary: overlay.summary,
            metrics: {
                ...(strengthResult.strengthDetail.metrics || {}),
                classical_strength_overlay: 1
            }
        }
        : strengthResult.strengthDetail;
    ...
    return {
        ...
        strongWeak: '身强',
        strengthBand: '偏强',
        totalStrengthScore: Math.max(Number(strengthResult.totalStrengthScore || 0), overlay.score),
        strengthDetail,
        strengthBasis: `${strengthResult.strengthBasis || ''} ${overlay.text}`.trim()
    };
}
...
function buildCompleteBaziDetail({ baZi, yun, isMale = true, currentYear = new Date().getFullYear() }) {
    ...
    const strengthResult = applyClassicalStrengthOverlay(
        BaziRuleEngine.calculateStrength(dayGan, gansArr, zhisArr),
        classicalPattern
    );
