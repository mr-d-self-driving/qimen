...
function isOpeningTargetTomb({ incomingZhi, targetElement, stateReport, rel }) {
  if (!targetElement) return false;
  const tombBranch = ELEMENT_TOMB[targetElement];
  if (!tombBranch || R.ZHI_CHONGS[incomingZhi] !== tombBranch) return false;
  if (rel.partner_zhi !== tombBranch) return false;
  return stateReport.shishen_assessments.some((item) =>
    item.is_in_tomb && item.zhi === tombBranch && item.pillar === rel.partner_pillar
  );
}
...
function identifyMechanisms({
  ...
  for (const rel of zhiRels) {
    ...
    if (rel.type === '六冲') {
      // 开墓库优先（只要冲中了目标的墓支）
      if (
        isOpenTomb(incomingZhi, stateReport.overall_stability, targetElement) ||
        isOpeningTargetTomb({ incomingZhi, targetElement, stateReport, rel })
      ) {
        mechanisms.push({
          type: MECH.KAI_MU,
...
function assessPillarImpact({
  ...
  // 是否真正激活目标元素（有力 + 作用于目标柱）
  const activatesTarget = mechanisms.some(
    m => targetPillarNames.has(m.target_pillar) &&
 ...
 */
function deriveStateChange({ stateReport, targetSpec, dayunImpact, liunianImpact, targetPillarNames }) {
  ...
  if (top.type === MECH.FAN_YIN) {
    return {
      ...
      is_activated: true,
      event_type: EVENT.TURMOIL,
      auspice_direction: '反吟天克地冲，结构撕裂，为大凶象',
    };
  }
 ...
 */
function formatMechanismForPrompt(m) {
  const check = m.vigor_check || {};
  return `${m.type}：${m.description}（引动强度 ${m.effective_strength}，trigger_vigor ${check.trigger_vigor ?? '未知'}，target_vigor ${check.target_vigor ?? '未知'}，vigor_check.is_effective=${check.is_effective ? 'true' : 'false'}，direction=${check.direction || 'unknown'}）`;
}

function formatImpactForPrompt(label, impact, maxMechanisms) {
  const lines = [
    `【${label}分析】${impact.gan}${impact.zhi}（${impact.twelve_phase}，trigger_vigor ${impact.trigger_vigor}，activates_target=${impact.activates_target ? 'true' : 'false'}）`
  ];
  if (impact.gaitou_jiejiao !== 'neutral' && impact.gaitou_jiejiao !== 'same') {
    lines.push(impact.gaitou_jiejiao_note);
  }
  const mechanisms = impact.mechanisms.slice(0, maxMechanisms);
  if (mechanisms.length) mechanisms.forEach((m) => lines.push(`- ${formatMechanismForPrompt(m)}`));
  else lines.push('- 未检测到直接作用于目标元素的有效机制。');
  return lines.join('\n');
}

function formatDynamicReportForPrompt(report, options = null) {
  if (!options) {
    const lines = [
      '【大运流年动态分析】',
  ...
  const isMajorWindow = !!(report.dayun_impact?.activates_target && report.liunian_impact?.activates_target);
  const lines = [
    '【大运流年动态分析】',
    formatImpactForPrompt('大运', report.dayun_impact, maxMechanismsPerPillar),
    '',
    formatImpactForPrompt('流年', report.liunian_impact, maxMechanismsPerPillar),
    '',
    '【目标元素引动结论】',
    `基准状态：${report.target_trigger.base_state}`,
    `状态变化：${report.target_trigger.state_change}`,
    `新稳定性：${report.target_trigger.new_stability}`,
    `target_trigger.is_activated=${report.target_trigger.is_activated ? 'true' : 'false'}`,
    `event_type=${report.target_trigger.event_type || '无'}`,
  ];
  if (includeMajorWindowFlag) lines.push(`is_major_window=${isMajorWindow ? 'true' : 'false'}`);
  if (report.target_trigger.auspice_direction) lines.push(`吉凶方向参考：${report.target_trigger.auspice_direction}`);
  return lines.join('\n');
}
