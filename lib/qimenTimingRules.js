...
function buildTimingPromptSection(timingAnalysis) {
  ...
  const scanLines = scanCandidates.length
    ? scanCandidates.slice(0, 6).map((item, index) => `${index + 1}. ${item.date}${item.time_branch ? ` ${item.time_branch}时` : ''}：${item.basis || item.trigger || '触发应期规则'}，置信度${item.confidence || 'medium'}。`).join('\n')
...
输出要求：
- analysis.dynamic_timing 必须说明“为什么按这个应期规则看”以及“哪些窗口值得观察”。
- domain_view.timing 必须优先使用上方前 1-3 个未来触发窗口。
