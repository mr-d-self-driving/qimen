...
function isValidChartInput({ dayGan, gans, zhis, monthZhi } = {}) {
  return Boolean(
    GAN_WUXING[dayGan]
    && Array.isArray(gans)
    && gans.length === 4
    && gans.every((gan) => GAN_WUXING[gan])
    && Array.isArray(zhis)
    && zhis.length === 4
    && zhis.every((zhi) => ZHI_CANGGAN[zhi])
    && ZHI_CANGGAN[monthZhi],
  );
}
...
function scoreFollowCandidates(input) {
  ...
  if (!isValidChartInput({ dayGan, gans, zhis, monthZhi })) {
    return [createInvalidCandidate()];
  }
