...
function scoreSingleImage(input) {
  ...
  return finalizeCandidate({
    ...
    match_score: sameCampDominancePoints + monthCommandPoints + purityPoints,
    dimensions: {
    ...
    reason_codes: targetRatio >= 0.55 ? ['TARGET_QI_DOMINANT'] : [],
    yongshen_strategy: 'FOLLOW_FORCE',
  });
}
...
function scoreTransformationCandidates(input) {
  ...
  return [1, 3].flatMap((partnerIndex) => {
    ...
    const jealousCombine = gans.filter((gan, index) =>
      index !== 2 && gan === partner,
    ).length > 1;
    const noJealousCombinePoints = jealousCombine ? 0 : 10;
    const intactPoints = targetRatio >= 0.3 ? 10 : targetRatio > 0 ? 5 : 0;
    const rawScore =
      25
      + monthCommandPoints
      + branchSupportPoints
    ...
    return [finalizeCandidate({
      ...
      subtype: `${dayGan}${partner}化${targetElement}`,
      target_element: targetElement,
      match_score: jealousCombine ? Math.min(79, rawScore) : rawScore,
      dimensions: {
      ...
      penalties: jealousCombine ? ['JEALOUS_COMBINE_CAP_79'] : [],
      reason_codes: [
        'ADJACENT_STEM_COMBINE',
        ...(jealousCombine ? ['JEALOUS_COMBINE'] : []),
      ],
...
function scoreMixedFollowCandidate(input) {
  ...
  const isDmRootless = dmSupport.branch_same_roots === 0;
  const hasVisibleSupport = dmSupport.exposed_same > 0 || dmSupport.exposed_seal > 0;
  ...
  if (
    ...
    || targetCamps.some((item) => item.ratio >= 0.5)
  ) {
    return null;
  }
