...
function scoreFollowCandidates(input, { followWeakV2 = true } = {}) {
  ...
  return relations.map(({ subtype, target, reason }) => {
    ...
    const targetPrimaryRoots = countPrimaryBranchRoots(zhis, target);
    const targetBranchRoots = countSupportingBranches(zhis, target);
    ...
    const earthSupportOverwhelmedByWoodKilling =
      followWeakV2
      && subtype === '从官杀格'
      && !dmSupport.is_rootless
      && dmSupport.dm_element === '土'
      && target === '木'
      && targetBranchRoots >= 3
      && exposedTarget >= 1
      && primaryDmRoots <= 1
      && primarySealRoots === 0
      && wuxingScores[outputElement] === 0;
    const relaxedOverride = followWeakV2
      && !strictOverride
      && (!anchoredSupport || earthSupportOverwhelmedByWoodKilling)
      && isGlobalDominant
      && targetRatio >= relaxedRatioMin
      && followChainRatio >= relaxedChainMin;
    const allowOverride = strictOverride || relaxedOverride;
    const weakFollowPoints = earthSupportOverwhelmedByWoodKilling ? 35 : (relaxedOverride ? 25 : 0);
    const effectiveVisibleSupportPenalty = relaxedOverride ? 0 : visibleSupportPenalty;
    const resolvedSubtype = relaxedOverride && subtype === '从官杀格' && (exposedTarget >= 2 || earthSupportOverwhelmedByWoodKilling)
      ? '从杀格'
      : subtype;
    ...
    if (earthSupportOverwhelmedByWoodKilling) {
      reasonCodes.push('EARTH_SUPPORT_OVERWHELMED_BY_WOOD_KILLING');
    }
    ...
    return finalizeCandidate({
      ...
      dimensions: {
        ...
        target_primary_roots: targetPrimaryRoots,
        target_branch_roots: targetBranchRoots,
      },
      penalties,
...
function scoreSingleImage(input) {
  ...
  const wealthDrainPenalty = wealthRatio >= 0.18 && sameCampBranchFramePoints === 0 && completeWoodFramePoints === 0 ? 10 : 0;
