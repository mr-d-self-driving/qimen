...
test('frontend copy protocol and output contract avoid large JSON templates', () => {
  ...
  assert.match(combined, /每条 28-70 字/);
...
test('report schema uses compact M3 contract with legacy fields removed', () => {
  ...
  assert.match(section, /support_factors：有利因素。输出 tone、summary、items\[\]/);
  assert.match(section, /constraint_factors：不利因素。输出 tone、summary、items\[\]/);
  assert.match(section, /decision 给出推进\/观望等待\/暂缓撤退三选一/);
  assert.match(section, /翻译层三段只输出：symbol、palace、tone、reading/);
  assert.match(section, /M3 全段不要输出 label、key_components、user_implication、verdict、evidence/);
