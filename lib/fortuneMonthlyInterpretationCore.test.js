...
test('dimension-specific fewshots swap with dimension', () => {
  const wealthPrompt = buildMonthlyInterpretationPrompt(profile, monthlyData, 'wealth');
  const lovePrompt = buildMonthlyInterpretationPrompt({ ...profile, gender: 'F' }, monthlyData, 'love');
