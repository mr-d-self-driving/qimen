...
function detectNamedFormation(formation, palaces, intent = {}) {
  ...
  if (type === 'san_qi_ru_mu') {
    ...
    return hit(palaces.find((p) => normalizeText(p.sky) === formation.qi && formation.palaceIndices.includes(p.index)));
  }
  if (type === 'zhi_shi_san_qi') {
    const SAN_QI = new Set(['乙', '丙', '丁']);
    return hit(palaces.find((p) => p.isZhiShi && SAN_QI.has(normalizeText(p.sky))));
  }
  ...
  if (type === 'wu_bu_yu_shi') {
    ...
    return flag(Boolean(hourEl && dayEl && CONTROLS[hourEl] === dayEl));
  }
  if (type === 'fu_gan_ge') {
    const dayStem = intent.dayStem || palaces.find((p) => p.isDayStem)?.earth;
  ...
  if (type === 'yu_nv_shou_men') {
    return hit(palaces.find((p) => p.isZhiShi && normalizeText(p.sky) === '丁'));
  }
