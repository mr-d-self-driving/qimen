...
test('annual cache version is bumped for P2-A shensha qi weighting', () => {
  assert.equal(ANNUAL_SCORE_CACHE_VERSION, 'annual-score-v6-p2a-shensha-qi');
});
...
test('annual tiaohou bonus does not lift score above favorable Kong Wang cap', () => {
  const result = calculateAnnualScore({
    ...baseProfile,
    ri_zhu: '甲午',
    nian_zhu: '庚午',
  ...
  assert.equal(result.kongwang_type, 'xiyong');
  ...
  assert.equal(tiaohouLayer.label, '调候修正层');
  assert.ok(result.annual_score <= 85, 'tiaohou should be applied before favorable Kong Wang cap');
...
test('layer3 supports suide and suide-he as separate mutually exclusive signals', () => {
  ...
  assert.match(suide.suiyun_relations, /岁德（流年干=年干）\+\d+（base=5/);
  ...
  assert.match(suideHe.suiyun_relations, /岁德合（流年干合年干化土）\+\d+（base=4/);
});
...
test('annual dayun background is lighter than liunian trigger in heavenly-stem scoring', () => {
  const result = calculateAnnualScore({
    ...baseProfile,
    day_zhi: '亥',
    month_zhi: '未',
  ...
  assert.match(result.layer1_detail, /大运天干\[庚\]七杀为忌神-7/);
  assert.match(result.layer2_detail, /流年天干\[壬\]偏印用神\+14/);
  assert.equal(result.layer1_score + result.layer2_score, 7);
});

test('annual layer ranges cap dayun at 15 and liunian at 28', () => {
  const result = calculateAnnualScore({
    ...baseProfile,
    day_zhi: '午',
    month_zhi: '辰',
    ...
    dayun_gz: '壬午',
    liunian_gz: '壬午',
    favorable_elements: ['印星', '食伤'],
    unfavorable_elements: [],
  });

  assert.ok(result.layer1_score <= 15);
  assert.ok(result.layer2_score <= 28);
});
...
test('tianyi and wenchang are weighted as auxiliary shensha instead of fixed auspicious boosts', () => {
  ...
  assert.match(tianyiUnfavorable.suiyun_relations, /天乙贵人\+0（base=5/);
  assert.match(tianyiUnfavorable.suiyun_relations, /tier=忌神/);
  assert.equal(tianyiUnfavorable.shensha_signals.tianyi_delta, 0);
  ...
  assert.match(tianyiFavorable.suiyun_relations, /天乙贵人\+4（base=5/);
  assert.equal(tianyiFavorable.shensha_signals.tianyi_delta, 4);
  ...
  assert.match(wenchangUnfavorable.suiyun_relations, /文昌贵人\+0（base=3/);
  assert.match(wenchangUnfavorable.suiyun_relations, /tier=忌神/);
  assert.equal(wenchangUnfavorable.shensha_signals.wenchang_delta, 0);
  ...
  assert.match(wenchangFavorable.suiyun_relations, /文昌贵人\+1（base=3/);
  assert.equal(wenchangFavorable.shensha_signals.wenchang_delta, 1);
});
