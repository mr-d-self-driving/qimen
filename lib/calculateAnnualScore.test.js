...
test('annual cache version is bumped for P1 dayun liunian weights', () => {
  assert.equal(ANNUAL_SCORE_CACHE_VERSION, 'annual-score-v4-p1-dayun-liunian');
});
...
test('annual tiaohou bonus does not lift score above favorable Kong Wang cap', () => {
  ...
  assert.ok(result.score_hits.layers.some(layer => layer.layer === 'tiaohou'));
