const MONTHLY_INTERPRETATION_PROMPT_VERSION = 'monthly-interpretation-v2-evidence';
...
只输出合法 JSON，禁止任何 markdown 包裹。
{
  ...
  "basis": ["2-3条判断依据，每条12-28字，直接说明本月判断主要来自什么信号"],
...
function pickMonthlyInterpretationFields(data = {}) {
  return {
    ...
    basis: Array.isArray(data.basis) ? data.basis.slice(0, 3).map(item => String(item || '')).filter(Boolean) : [],
