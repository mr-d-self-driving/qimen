...
const CATEGORY_RULES = [
  ...
  },
  {
    ...
    words: ['健康', '疾病', '病情', '手术', '治疗', '医生', '药', '体质', '脏腑', '运动比赛'],
    subcategories: [
...
function confidenceFromScores(primary, secondary = 0) {
  ...
  if (primary >= 4 && primary - secondary >= 2) return 'medium';
  return 'low';
}
...
3. 如果问题缺少时间范围、对象或判断目标，branch 返回 clarify。
4. 不要给建议，不要解读，不要输出 markdown。
...
few-shots：
1. 用户问："我未来五年适合创业还是打工？"
返回：{"branch":"bazi","category":"career_business","subcategory":"entrepreneurship_vs_job","confidence":"high","reason":"询问长期职业路径和创业打工取舍，属于八字宏观命局判断","followupQuestion":""}

2. 用户问："A公司和B公司的offer我该选哪个？"
返回：{"branch":"qimen","category":"career_business","subcategory":"job_search","confidence":"high","reason":"询问具体offer选择，属于奇门短期事件决策","followupQuestion":""}

3. 用户问："我今年事业运不好，这个offer要不要接？"
返回：{"branch":"hybrid","category":"career_business","subcategory":"current_job_change","confidence":"high","reason":"同时包含今年事业运的宏观背景和offer是否接受的具体事件","followupQuestion":""}

4. 用户问："我先天体质哪些地方要注意？"
返回：{"branch":"bazi","category":"health_action","subcategory":"constitution","confidence":"high","reason":"询问先天体质结构，属于八字宏观健康判断","followupQuestion":""}

5. 用户问："明天去表白能成功吗？"
返回：{"branch":"qimen","category":"relationship","subcategory":"pursuit_dating","confidence":"high","reason":"询问明天表白成败，属于奇门具体事件判断","followupQuestion":""}

6. 用户问："帮我看看最近怎么样"
返回：{"branch":"clarify","category":"general","subcategory":"vague","confidence":"low","reason":"缺少具体领域、时间范围和判断目标","followupQuestion":"你是想看长期趋势，还是判断眼前某一件具体事情？如果是具体事，请补充对象、时间和想判断的结果。"}
...
只返回 JSON：
{"branch":"bazi|qimen|hybrid|clarify","category":"枚举category","subcategory":"枚举subcategory或空字符串","confidence":"low|medium|high","reason":"一句话说明","followupQuestion":"clarify时的追问，否则空字符串"}`;
}
