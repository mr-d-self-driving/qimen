...
function locateShishen(pillars, targetShishens, dayStem) {
  ...
  for (const pillar of pillars) {
    ...
    const mainGan = pillar.hidden_stems?.[0] ?? '';
    if (mainGan) {
    ...
    // 藏干（余气/中气）
    for (const hg of (pillar.hidden_stems ?? []).slice(1)) {
...
function assessOneShishen({ loc, pillars, dayStem, monthZhi }) {
  ...
  const vigor = getVigor(phase, is_kong);
...
function assessGongwei({ gongwei, pillar, targetShishens, targetElement, pillars, dayStem, monthZhi }) {
  ...
  const seatGan = pillar.hidden_stems?.[0] ?? '';
...
const STATIC_CHECK_RULES = {
  ...
  '日支坐比劫 → 配偶宫被争夺，婚姻危机': ({ pillars, dayStem }) => {
    ...
    const mainGan = riPillar.hidden_stems?.[0] ?? '';
...
function deriveOverallStability(shishenAssessments, gongweiAssessments) {
  const allTags = new Set([
    ...shishenAssessments.flatMap(a => a.status_tags),
    ...gongweiAssessments.flatMap(a => a.status_tags),
  ]);

  // 宫位冲刑是动态触发信号，优先于星的弱死判断（因为即使星弱，宫冲仍能引发事件）
  if (allTags.has('宫位受冲') || allTags.has('被冲') || allTags.has('宫位受刑')) return 'dynamic';
  if (allTags.has('鸠占鹊巢')) return 'dynamic';
  if (allTags.has('被合化')) return 'damaged';
  if (allTags.has('死绝') && allTags.has('空亡')) return 'damaged';
  if (allTags.has('入墓') && !allTags.has('宫位受冲') && !allTags.has('被冲')) return 'buried';
  if (allTags.has('帝旺') && allTags.has('宫位稳固')) return 'strong';
  return 'stable';
}
...
function buildBaseState(shishenAssessments, gongweiAssessments, extraCheckResults) {
  ...
  for (const g of gongweiAssessments) {
    const key = g.status_tags.filter(t => ['宫位稳固', '宫位受冲', '宫位受刑', '宫位入墓', '鸠占鹊巢', '星宫得正'].includes(t));
 ...
 */
function formatStateReportForPrompt(report, options = null) {
  if (!options) {
    const lines = [
      `【原局状态评估】`,
      `整体稳定性：${report.stability_label}（${report.overall_stability}）`,
      `基准状态：${report.base_state}`,
      '',
  ...
  report.shishen_assessments.slice(0, maxShishenItems).forEach((item, index) => {
    const quant = includeQuantTags ? `（vigor ${item.vigor}，十二长生 ${item.twelve_phase}，标签 ${item.status_tags.join('/') || '无'}）` : '';
    lines.push(`${index + 1}. ${item.shishen}位于${item.pillar}柱${item.position}，${item.verdict}${quant}`);
  });

  report.gongwei_assessments.slice(0, maxGongweiItems).forEach((item, index) => {
    const quant = includeQuantTags ? `（宫位状态 ${item.status_tags.join('/') || '无'}，vigor ${item.vigor}）` : '';
    lines.push(`${report.shishen_assessments.slice(0, maxShishenItems).length + index + 1}. ${item.gongwei}${item.zhi}：${item.verdict}${quant}`);
  });

  if (!report.shishen_assessments.length && !report.gongwei_assessments.length) {
    lines.push('1. 未找到明确十神或宫位依据，只能按原局整体状态低置信判断。');
  }
