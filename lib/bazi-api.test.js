...
test('getGeJu promotes tomb-month exposed finance over base seal（汪学士型）', () => {
  const result = baziModule.getGeJu({
    year: ['甲', '子'],
    month: ['辛', '未'],
    day: ['辛', '酉'],
  ...
  assert.equal(result.geju, '正财格');
  assert.equal(result.basis, '主格候选');
  assert.equal(result.tenGod, '财');
  assert.equal(result.monthMainGan, '甲');
  assert.equal(result.patternCandidates[0].type, 'graveyard_exposed_tomb_finance');
  assert.equal(result.patternCandidates.some(item => item.type === 'base_month' && item.name === '偏印格'), true);
});

test('getGeJu promotes exposed hurt-output over graveyard peer main qi（假伤官型）', () => {
  const result = baziModule.getGeJu({
    year: ['己', '酉'],
    month: ['辛', '未'],
    day: ['戊', '辰'],
  ...
  assert.equal(result.geju, '伤官格');
  assert.equal(result.basis, '主格候选');
  assert.equal(result.tenGod, '伤');
  assert.equal(result.monthMainGan, '辛');
  assert.equal(result.patternCandidates[0].type, 'graveyard_peer_exposed_hurt_output');
  assert.equal(result.patternCandidates.some(item => item.type === 'base_month' && item.name === '羊刃格'), true);
});
...
test('getChengGe prefers QiongTong 丁辰庚金泄土 when no water killing is exposed（罗文干型）', () => {
  const result = baziModule.getChengGe({
    geju: '伤官格',
    basis: '月令本气',
    dayGan: '丁',
  ...
  assert.equal(result.chengGe, '穷通丁辰庚金泄土格');
  assert.equal(result.yongShenTenGod, '财');
  assert.equal(result.yongShen, '庚');
  assert.deepEqual(result.patternEffects.promotedShens, ['财']);
});
...
test('getChengGe recognizes 身重杀轻借财清格 before sha-seal fallback（刘运使型）', () => {
  const result = baziModule.getChengGe({
    geju: '七杀格',
    basis: '月令本气',
    dayGan: '丙',
  ...
  assert.equal(result.chengGe, '借财清格');
  assert.equal(result.chengGeResult, '成格');
  assert.equal(result.yongShenTenGod, '才');
  assert.equal(result.yongShen, '庚');
  assert.ok(result.patternEffects.methodTags.includes('借财清格'));
  assert.deepEqual(result.patternEffects.demotedShens, ['印', '枭']);
});

test('getChengGe recognizes 壬亥建禄伤官生财 instead of generic 建禄（靳云鹏型）', () => {
  const result = baziModule.getChengGe({
    geju: '建禄格',
    basis: '建禄特判',
    dayGan: '壬',
  ...
  assert.equal(result.chengGe, '建禄伤官生财格');
  assert.equal(result.yongShenTenGod, '伤');
  assert.equal(result.yongShen, '乙');
  assert.deepEqual(result.patternEffects.promotedShens, ['伤']);
  assert.deepEqual(result.patternEffects.assistantShens, ['财', '才']);
});

test('getChengGe keeps 壬亥建禄用官 when 庚合乙 removes hurt-output（王总兵型）', () => {
  const result = baziModule.getChengGe({
    geju: '建禄格',
    basis: '建禄特判',
    dayGan: '壬',
  ...
  assert.equal(result.chengGe, '建禄用官格');
  assert.equal(result.yongShenTenGod, '官');
  assert.equal(result.yongShen, '己');
});

test('getChengGe keeps exposed killing and seal as sha-seal pattern（何参政型）', () => {
  const result = baziModule.getChengGe({
    geju: '七杀格',
    basis: '月令本气',
    dayGan: '壬',
...
test('getChengGe recognizes 财轻比强用伤官化劫生财（汪学士型）', () => {
  const result = baziModule.getChengGe({
    geju: '正财格',
    basis: '主格候选',
    dayGan: '辛',
  ...
  assert.equal(result.chengGe, '财用伤官格');
  assert.equal(result.chengGeResult, '成格');
  assert.equal(result.yongShenTenGod, '伤');
  assert.equal(result.yongShen, '壬');
  assert.deepEqual(result.patternEffects.protectedShens, ['伤', '财', '才']);
  assert.ok(result.patternEffects.methodTags.includes('化劫生财'));
});

test('getChengGe recognizes repeated food output generating rooted wealth（杨侍郎型）', () => {
  const result = baziModule.getChengGe({
    geju: '偏财格',
    basis: '月令本气',
    dayGan: '庚',
  ...
  assert.equal(result.chengGe, '财喜食生格');
  assert.equal(result.chengGeResult, '成格');
  assert.equal(result.yongShenTenGod, '食');
  assert.equal(result.yongShen, '壬');
  assert.equal(result.xianShen, '才');
  assert.ok(result.patternEffects.methodTags.includes('财用食生'));
});

test('getChengGe recognizes 财格食神透根生财（柯文敏型）', () => {
  const result = baziModule.getChengGe({
    geju: '偏财格',
    basis: '月令本气',
    dayGan: '甲',
...
test('getChengGe recognizes 癸卯食神当令财透生财（梁丞相型）', () => {
  const result = baziModule.getChengGe({
    geju: '食神格',
    basis: '月令本气',
    dayGan: '癸',
  ...
  assert.equal(result.chengGe, '食神生财格');
  assert.equal(result.yongShenTenGod, '食');
  assert.equal(result.yongShen, '乙');
  assert.deepEqual(result.patternEffects.promotedShens, ['食']);
  assert.deepEqual(result.patternEffects.assistantShens, ['财', '才']);
...
test('buildCompleteBaziDetail applies source-backed 金木间隔自旺 as a classical strength overlay（杨博型）', () => {
  const baZi = Solar.fromYmdHms(1509, 6, 11, 8, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.pattern_analysis.final_pattern.name, '金木间隔');
  assert.equal(result.strong_weak, '身强');
  assert.match(result.strength_basis, /金木间隔/);
});
...
test('hasLatestEngineCache invalidates pre-image-analysis engine caches', () => {
  ...
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.19' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.20' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.21' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.22' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.23' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.24' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.26' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.27' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.28' } }), true);
});
...
test('buildCompleteBaziDetail keeps winter wood variation on fire and wealth（职业妇女型）', () => {
  const baZi = Solar.fromYmdHms(1645, 12, 23, 18, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(0);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
    isMale: false,
  ...
  assert.equal(result.chengge_detail.chengGe, '食神生财格');
  assert.equal(result.five_shens.yong, '伤官');
  assert.equal(result.five_shens.xi.includes('偏财'), true);
  assert.equal(result.unfavorable_gods.includes('正印'), true);
  assert.ok(result.decision_chain.some(step => /忌水木/.test(step.reason)));
});

test('buildCompleteBaziDetail prefers exposed Bing for cold Jia wood facing sunlight（寒木向阳型）', () => {
  const baZi = Solar.fromYmdHms(1530, 12, 17, 4, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.chengge_detail.chengGe, '寒木向阳格');
  assert.equal(result.five_shens.yong, '食神');
  assert.equal(result.five_shens.xi.includes('比肩'), true);
  assert.equal(result.unfavorable_gods.includes('七杀'), true);
  assert.ok(result.decision_chain.some(step => /寒木向阳/.test(step.reason)));
});

test('buildCompleteBaziDetail keeps earth tri-punishment hurt-output from L0 force override（农妇型）', () => {
  const baZi = Solar.fromYmdHms(1636, 11, 3, 14, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(0);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.geju, '伤官格');
  assert.equal(result.chengge_detail.chengGe, '三刑土埋用印格');
  assert.equal(result.five_shens.yong, '偏印');
  assert.equal(result.favorable_gods.includes('正官'), true);
  assert.equal(result.unfavorable_gods.includes('伤官'), true);
  assert.ok(result.decision_chain.some(step => /三刑土埋|土为忌/.test(step.reason)));
});

test('buildCompleteBaziDetail recognizes wet earth overdrain using Wu fire（董中堂型）', () => {
  const baZi = Solar.fromYmdHms(1500, 4, 12, 12, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.chengge_detail.chengGe, '湿土过泄用午火格');
  assert.equal(result.five_shens.yong, '正印');
  assert.equal(result.favorable_gods.includes('正印'), true);
  assert.equal(result.favorable_gods.includes('七杀'), false);
  assert.equal(result.unfavorable_gods.includes('七杀'), true);
  assert.ok(result.decision_chain.some(step => /湿土过泄|用午火|水木不见/.test(step.reason)));
});

test('buildCompleteBaziDetail keeps Dai Shangshu from generic Ding-Si Hai-officer use（戴尚书型）', () => {
  const baZi = Solar.fromYmdHms(1515, 5, 13, 20, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.chengge_detail.chengGe, '夫健怕妻取印格');
  assert.equal(result.five_shens.yong, '偏印');
  assert.equal(result.favorable_gods.includes('偏印'), true);
  assert.equal(result.favorable_gods.includes('正官'), false);
  assert.equal(result.unfavorable_gods.includes('正官'), true);
  assert.equal(result.favorable_gods.includes('正财'), false);
  assert.ok(result.decision_chain.some(step => /夫健怕妻|巳亥冲|东方木地/.test(step.reason)));
});

test('buildCompleteBaziDetail keeps early-spring Bing fire on visible Yi seal（朱中堂型）', () => {
  const baZi = Solar.fromYmdHms(1671, 3, 4, 14, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.chengge_detail.chengGe, '去浊留清正印格');
  assert.equal(result.five_shens.yong, '正印');
  assert.equal(result.favorable_gods.includes('正印'), true);
  assert.equal(result.five_shens.xi.includes('七杀'), true);
  assert.equal(result.favorable_gods.includes('正财'), false);
  assert.equal(result.unfavorable_gods.includes('偏财'), true);
  assert.ok(result.decision_chain.some(step => /火虚木嫩|亥水流金|去浊留清/.test(step.reason)));
});

test('buildCompleteBaziDetail keeps food-earth supportive in official-to-seal wealth-use（金状元型）', () => {
  const baZi = Solar.fromYmdHms(1615, 11, 24, 20, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.geju, '正官格');
  assert.equal(result.chengge_detail.chengGe, '化官为印格');
  assert.equal(result.five_shens.yong, '正财');
  assert.equal(result.favorable_gods.includes('正财'), true);
  assert.equal(result.unfavorable_gods.includes('伤官'), false);
  assert.equal(result.five_shens.xi.includes('伤官'), true);
  assert.ok(result.decision_chain.some(step => /化官为印|用财损印|食伤生财/.test(step.reason)));
});

test('buildCompleteBaziDetail recognizes fake hurt-output with arched metal use（假伤官型）', () => {
  const baZi = Solar.fromYmdHms(1549, 7, 24, 20, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.geju, '伤官格');
  assert.equal(result.chengge_detail.chengGe, '假伤官格');
  assert.equal(result.five_shens.yong, '伤官');
  assert.equal(result.five_shens.xi.includes('比肩'), true);
  assert.equal(result.unfavorable_gods.includes('正印'), true);
  assert.equal(result.favorable_gods.includes('七杀'), false);
  assert.ok(result.decision_chain.some(step => /假伤官|暗拱金局/.test(step.reason)));
});
...
test('buildCompleteBaziDetail applies source-backed strength overlay for 伤官吐秀两辰拱水（臬藩型）', () => {
  const baZi = Solar.fromYmdHms(1520, 3, 22, 0, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.pattern_analysis.final_pattern.name, '伤官格');
  assert.equal(result.pattern_analysis.classical_pattern.aliases.includes('伤官吐秀'), true);
  assert.equal(result.strong_weak, '身强');
  assert.equal(result.five_shens.yong, '伤官');
  assert.equal(result.favorable_gods.includes('伤官'), true);
  assert.match(result.strength_basis, /两辰拱水|伤官吐秀/);
  assert.ok(result.decision_chain.some(step => /伤官吐秀|卯木为用/.test(step.reason)));
});

test('buildCompleteBaziDetail applies source-backed strength and yong overlay for 去火存金（莫宝斋型）', () => {
  const baZi = Solar.fromYmdHms(1521, 5, 27, 22, 30, 0).getLunar().getEightChar();
  const yun = baZi.getYun(1);
  const result = buildCompleteBaziDetail({
    baZi,
    yun,
  ...
  assert.equal(result.pattern_analysis.final_pattern.name, '财旺生官');
  assert.equal(result.strong_weak, '身强');
  assert.equal(result.five_shens.yong, '偏印');
  assert.equal(result.favorable_gods.includes('偏印'), true);
  assert.equal(result.favorable_gods.includes('偏财'), false);
  assert.match(result.strength_basis, /去火存金|日主之气已贯/);
  assert.ok(result.decision_chain.some(step => /去火存金|体用不伤/.test(step.reason)));
});
