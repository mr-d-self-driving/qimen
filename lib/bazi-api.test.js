...
test('getChengGe keeps separated food and seal when a wealth month has no hidden officer（吴榜眼型）', () => {
  const result = baziModule.getChengGe({
    geju: '正财格',
    basis: '月令本气',
    dayGan: '戊',
  ...
  assert.notEqual(result.chengGe, '财格去食护官格');
});
...
test('hasLatestEngineCache invalidates pre-image-analysis engine caches', () => {
  ...
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.10' } }), true);
});
