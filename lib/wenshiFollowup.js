...
// 八字可重算的轴（首轮路由词汇；不是独立 capability 名）。
...
route_delta（仅 recompute 时给，用首轮路由词汇）：
  - analysis_mode: timing | pattern | character | status
  - time_scope: { type, start_year, end_year }（问具体年份/年段时给）
  - target: { category }（换领域 / 换十神靶时给）
  - secondary_mode（可选）
`}
...
function normalizeRouteDelta(raw) {
  const r = raw && typeof raw === 'object' ? raw : {};
  const out = {};
  if (BAZI_ROUTE_MODES.has(r.analysis_mode)) out.analysis_mode = r.analysis_mode;
  if (r.time_scope && typeof r.time_scope === 'object') out.time_scope = r.time_scope;
  if (r.target && typeof r.target === 'object') out.target = r.target;
  if (BAZI_ROUTE_MODES.has(r.secondary_mode)) out.secondary_mode = r.secondary_mode;
  return out;
}
