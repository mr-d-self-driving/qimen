...
function calculateLayer3({
  ...
  if (yimaZhi && yimaZhi === liunianZhi) {
    ...
    if (yimaTier === 'yong' || yimaTier === 'xi') { raw3 += 4; parts.push(`驿马（喜用方向）+4`); }
    else if (yimaTier === 'ji' || yimaTier === 'chou') { raw3 -= 2; parts.push(`驿马（忌仇方向）-2`); }
...
function calculateAnnualScore(profile) {
  ...
  const l3 = calculateLayer3({ riGan, yuanYearGan, dayunGan, dayunZhi, liunianGan, liunianZhi, yuanDayZhi, yuanYearZhi, shen });
  ...
  if (tiaohouHit) score_hits.layers.push({ layer: 'tiaohou', label: '调候层', score: 0, hits: [tiaohouHit] });
