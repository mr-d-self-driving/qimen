...
function buildAnnualScoreHits({
  ...
  return {
    ...
    summary: {
      ...
      debug: `base=70, rawBeforeCaps=${rawBeforeCaps}, layer1=${layer1.layer1_score}, layer2=${layer2.layer2_score}, layer3=${layer3.layer3_score}, caps=${capLabels.join('|') || 'none'}, final=${finalScore}`,
    },
    layers,
  };
}
...
function calculateLayer1({ riGan, dayunGan, dayunZhi, yuanDayZhi, yuanMonZhi, yuanYearZhi, fav, unfav }) {
  let raw1 = 0;
  ...
  if (inList(fav, dayunGanSS))        { raw1 += 9; parts.push(`大运天干[${dayunGan}]${dayunGanSS}为喜用+9`); }
  else if (inList(unfav, dayunGanSS)) { raw1 -= 9; parts.push(`大运天干[${dayunGan}]${dayunGanSS}为忌仇-9`); }
  ...
  if (hasSanXing(dayunZhi, yuanZhiSet)) {
    ...
    if (hasMonthHe6) {
      const s = heScore(liuheInfo.hua, riGan, fav, unfav, 5);
      raw1 += s;
      parts.push(`大运支[${dayunZhi}]与月支[${yuanMonZhi}]六合化${liuheInfo.hua}${s>0?'+':''}${s}`);
    }
    if (hasMonthHe3) {
      const s = heScore(sanheInfo.hua, riGan, fav, unfav, 4);
      raw1 += s;
...
function calculateLayer2({
  ...
  if (inList(fav, liunianGanSS))        { raw2 += 9;  parts.push(`流年天干[${liunianGan}]${liunianGanSS}喜用+9`); }
  else if (inList(unfav, liunianGanSS)) { raw2 -= 9;  parts.push(`流年天干[${liunianGan}]${liunianGanSS}忌仇-9`); }
  ...
  if (hasSanXing(liunianZhi, yuanZhiSet)) {
    ...
    if (haiTarget && yuanZhiSet.has(haiTarget)) {
      const t = zhiUseType(haiTarget, riGan, fav, unfav);
...
function calculateLayer3({
  ...
  if (tianyiStr.includes(liunianZhi)) {
    ...
    const pts = lnZhiType === '喜用' ? 10 : 8;
    raw3 += pts;
    parts.push(lnZhiType === '喜用'
      ? `天乙贵人+${pts}（固定吉神，喜用年额外+2）`
      : `天乙贵人+${pts}（固定吉神，不随${lnZhiType}打折）`);
  }
  ...
  if (wenchangZhi === liunianZhi) {
    ...
    const pts = lnZhiType === '喜用' ? 8 : 6;
    raw3 += pts;
    parts.push(lnZhiType === '喜用'
      ? `文昌贵人+${pts}（固定吉神，喜用年额外+2）`
      : `文昌贵人+${pts}（固定吉神，不随${lnZhiType}打折）`);
  }
...
function calculateAnnualScore(profile) {
  ...
  // ── 三层计算 ───────────────────────────────────────────────────────────────
  const l1 = calculateLayer1({ riGan, dayunGan, dayunZhi, yuanDayZhi, yuanMonZhi, yuanYearZhi, fav, unfav });
