...
test('buildMonthlyFortunePayload exposes monthly score for frontend month tab', () => {
  ...
  assert.equal(payload.score_hits.version, 'monthly-score-hits-v2');
...
test('monthly score cache version is bumped for P2-A shensha qi weighting', () => {
  assert.equal(MONTHLY_SCORE_CACHE_VERSION, 'monthly-score-v10-p2a-shensha-qi');
});
