...
test('complete spring wood frame overrides despite earth wealth noise（段祺瑞曲直型）', () => {
  const result = assessBaziImage({
    dayGan: '乙',
    gans: ['乙', '己', '乙', '癸'],
    zhis: ['丑', '卯', '亥', '未'],
  ...
  assert.ok(single.match_score >= 73);
  assert.equal(result.override_candidate?.subtype, '曲直格');
});
...
test('dominant wood killing can make weak earth support ineffective and form 从杀格（伍廷芳型）', () => {
  const result = assessBaziImage({
    dayGan: '己',
    gans: ['壬', '丁', '己', '乙'],
    zhis: ['寅', '未', '卯', '亥'],
  ...
  const follow = getCandidates(result).find((candidate) => candidate.subtype === '从杀格');

  assert.ok(follow, '应识别从杀格候选');
  assert.equal(follow.category, IMAGE_CATEGORY.FOLLOW_IMAGE);
  assert.equal(follow.override_normal_pattern, true);
  assert.equal(follow.override_scope, 'full');
  assert.ok(follow.reason_codes.includes('EARTH_SUPPORT_OVERWHELMED_BY_WOOD_KILLING'));
  assert.ok(follow.reason_codes.includes('FOLLOW_CHAIN_DOMINANT'));
  assert.equal(result.override_candidate?.subtype, '从杀格');
});
...
test('maps dominant single images for every element', () => {
  const charts = [
    ...
    ['壬', ['壬', '癸', '壬', '庚'], ['亥', '子', '申', '亥'], '子', '润下格（水局一气）', '水'],
  ];
...
test('water single image exposes 水局一气 alias for classical one-qi cases', () => {
  const result = assessBaziImage({
    dayGan: '癸',
    gans: ['癸', '癸', '癸', '癸'],
    zhis: ['亥', '亥', '丑', '丑'],
  ...
  assert.match(candidate.subtype, /润下格/);
  assert.match(candidate.subtype, /水局一气/);
  assert.equal(candidate.override_normal_pattern, true);
});
