 ...
 * }}
 */
function calculateTiaohouRatio(profile, flowGan, flowLabel = '流年') {
  ...
  if (flowGan === p1) {
    return {
      ratio: 1.08,
      tier: 'satisfied_p1',
      display: `${flowLabel}天干[${flowGan}]为命局第一调候用神，寒暖得宜，气候环境显著改善，运势有额外加成。`,
  ...
  if (favorable.includes(flowGan)) {
    return {
      ratio: 1.04,
      tier: 'satisfied',
      display: `${flowLabel}天干[${flowGan}]补足命局调候需求，寒暖燥湿趋于平衡，运势小幅加成。`,
  ...
  if (unfavorable.includes(flowGan)) {
    return {
      ratio: 0.93,
      tier: 'worsened',
      display: `${flowLabel}天干[${flowGan}]加剧命局气候偏枯，调候失衡，无论格局如何，运势底色折损。`,
