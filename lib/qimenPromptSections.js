...
- qimen_report.m1_conclusion：title、keyword、conclusion、actions。actions 固定 3 条，每条 28-70 字，动词开头，必须能落地到沟通、等待、催动、取证、避险、择时或调整动作。
...
- qimen_report.m3_inference：按 subject_state、target_state、environment_state、support_factors、constraint_factors、interaction_decision 六段输出。不要输出 label、key_components、user_implication、verdict、evidence。
- qimen_report.m4_guidance.environment_fengshui：写环境、方位、避让和原因。
- qimen_report.m4_guidance.timing_behavior：写时机窗口、等待条件、行动和避让动作；日期或时辰只能来自后端候选，不能自行新增。`;
}
...
1. m1_conclusion：结论先行。
   ...
   - keyword：4-12字核心判断词。
   - conclusion：35-80字总判。
   - actions：固定输出3条，动词开头，28-70字，从盘面风险反推，不写泛泛建议。
...
3. m3_inference：局象推演，必须按顺序包含六段，分三层。
   ...
   【翻译层】客观描述盘面，不作好坏判断，不说"有利""不利"：
   - subject_state：日干落宫状态。必须说明：日干临何宫、何门、何星、何神，旺衰如何；如有空亡/马星/入墓/受刑，如实点出，不下结论。
   - target_state：领域核心用神落宫状态。必须说明：用神（时干或领域主符号）落何宫、临何门，旺衰与空亡状态；用神逢空写"虚、未落实"，不判死。
   - environment_state：值使门与宏观环境状态。必须说明：值使门是何门、流程顺滞；值符落宫有何信号；描述"管道"和外部条件，不评价结果。

   【提炼层】综合翻译层，讲人话，不写门/星/神符号（必要时括号注释）：
   - support_factors：有利因素。输出 tone、summary、items[]。summary 用一句话概括正向支撑；items 固定 1-3 项，每项包含 name、impact。只写真实有力的支撑，不夸大弱支撑。
   - constraint_factors：不利因素。输出 tone、summary、items[]。summary 用一句话概括核心制约；items 固定 1-3 项，每项包含 name、impact。不恐吓，不写"必败"。

   【拍板层】综合前五段给出方向性结论：
   - interaction_decision：生克决断。必须引用 backend relations 的 subject_symbol ↔ target_symbol；decision 给出推进/观望等待/暂缓撤退三选一，不模棱两可（40字以内）；reason 说明依据；不得自行发明后端 relations 未出现的宫位关系。须包含 subject_symbol、target_symbol、tone、decision、reason。

   翻译层三段只输出：symbol、palace、tone、reading。reading 必须引用至少一种盘面依据：落宫、门、星、神、天地盘干、空亡、马星、有名格、日时生克或应期候选。
   M3 全段不要输出 label、key_components、user_implication、verdict、evidence；这些字段已下线或由前端固定标题/旧记录 fallback 处理。

4. m4_guidance：开运指南。
   - environment_fengshui：direction、avoid_direction、environment_advice、reason（必须说明盘面依据）。
   - timing_behavior：best_window（只能基于后端应期候选或写"暂无明确窗口"）、wait_until、action、avoid_action、reason。
