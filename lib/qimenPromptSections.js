...
8. 用户态表达必须用人话，不要原样展示“乾6宫单宫能量、日干/时干主客生克、qimen-score-v4”等后端技术标签。
9. 若后端证据摘要中有“有名格层明细”命中且形成净影响，必须在 layer_reviews、analysis.pattern、display_blocks.support/risk 或 domain_view.axes.evidence 中至少一处承接其现实含义；若未承接，视为审计不完整。
...
总原则：
1. 不要展示后端技术词：不得原样写 backend_pre_score、audit_delta、qimen-score-v4、乾6宫单宫能量、日干/时干主客生克。
...
- summary.conclusion：一句话说明当前倾向和不确定性。
- summary.score：不要输出，由后端拼接。
- summary.score_basis：positive_signals 和 negative_signals 各 1-3 条，score_logic 不提后端、模型、审计、程序。
- display_blocks.situation：当前局势，80-160字。
- display_blocks.support：真实支撑，60-140字，不夸大弱支撑。
- display_blocks.risk：主要风险，60-140字，不恐吓。
- display_blocks.timing：必须基于后端应期候选；无候选则写暂无明确窗口。
- advice.strategy：2-4条，动词开头，现实可执行。
- advice.lucky_tips.time：必须来自后端候选或写暂无明确。`;
}
