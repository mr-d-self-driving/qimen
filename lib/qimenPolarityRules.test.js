...
test('sports competition flips hurt door and white tiger into positive with risk', () => {
  ...
  assert.match(section, /qimen_report\.m2_basis\.yongshen_cards/);
  assert.match(section, /qimen_report\.m3_inference\.support_factors\/constraint_factors/);
  assert.doesNotMatch(section, /domain_view/);
});
