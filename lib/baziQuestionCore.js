...
function inferBaziRouteFromQuestion(question, route = {}) {
  ...
  return {
    ...route,
    analysis_mode: route.analysis_mode || route.analysis_mode_hint || analysisMode,
    secondary_mode: route.secondary_mode ?? route.secondary_mode_hint ?? secondaryMode,
    needs_time_scan: route.needs_time_scan ?? route.needs_time_scan_hint ?? analysisMode === 'timing',
    time_scope: route.time_scope || route.time_scope_hint || timeScope
  };
}

function resolveConcreteTimeScope(timeScope = {}, currentYear, { analysisMode, nowYear } = {}) {
...
严格返回统一 envelope，不要包含 trigger_vigor、activation_strength、activates_target、is_major_window、vigor_check 等 pipeline 内部计算字段（这些由系统组装层处理）：
{
  ...
  "dynamic_context": ${mode === 'status' || mode === 'timing' ? '{"current_climate": ""}' : 'null'},
...
function buildModeAnalysisSchema(mode, secondaryMode) {
  if (mode === 'status') {
    ...
    "dayun_reading": "大运如何影响本命题的叙述（引用系统断语，不回显数值字段）",
    "liunian_reading": "流年的触发效果叙述（是否能推进/合稳/逆向催动，以及程度）",
  ...
  if (mode === 'pattern') {
    ...
    "structural_supports": [],
    "structural_risks": [],
    "verdict": ""${secondaryMode === 'status' ? `,
    "current_status_note": "secondary=status 时说明当前大运流年是否适合推进"` : ''}
  }`;
  }
  if (mode === 'character') {
    return `{
...
3. mode_analysis 使用 pattern 结构，至少包含 domain_fit、capacity、structural_risks、suitable_forms；
4. 若 secondary_mode=status，dynamic_context 中说明当前阶段气候，但不要把它写成应期预测；
5. 不要写“必然成功/必然失败”。
...
4. 如果 secondary_mode=status，dynamic_context 中说明当前阶段状态与变化，但不要扩展成未来应期；
...
1. 先说明原局是否具备该问题领域的底盘；
2. 再按候选年份解释为什么强、中、弱；
3. best_windows 只能来自候选年份断语包，不要新增未经扫描的年份；每个窗口的 verdict 和 mechanisms_text 用自然语言叙述，不回显数值字段；
4. 对 estimated/fallback_current 的大运定位要降低置信；
5. 若候选年份为空，降级说明当前资料不足以做长程应期遍历。
...
function normalizeBaziQuestionOutput(raw = {}, { question = '', route = {}, pipelineResult = null } = {}) {
  ...
  const dynamicContext = raw.dynamic_context || {};
  ...
  if (pipelineResult && raw.meta?.analysis_mode === 'timing' && pipelineResult.timingCandidates?.length) {
    ...
    const assembledWindows = pipelineResult.timingCandidates.map((candidate) => {
      const llmWindow = llmWindows.find((w) => String(w.year) === String(candidate.year)) || {};
      return {
        year: candidate.year,
        ganzhi: candidate.ganzhi,
        dayun_ganzhi: candidate.dayun_ganzhi,
        quality: candidate.quality,
        is_major_window: candidate.is_major_window || false,
        event_type: llmWindow.event_type || candidate.event_type || '',
  ...
  return {
    ...
    question,
    branch: 'bazi',
    category: route.category || meta.category || 'general',
    subcategory: route.subcategory || meta.subcategory || '',
    route,
    analysis: {
      tensor: assembledModeAnalysis.current_climate || dynamicContext.current_climate || raw.dayun_liunian?.current_climate || '',
      yong_shen: Array.isArray(foundation.core_stars) ? foundation.core_stars.join('、') : raw.ming_ju_analysis?.core_shishen || '',
      bazi_insight: foundation.capacity_level || assembledModeAnalysis.domain_fit || raw.ming_ju_analysis?.capacity || '',
      ...
      dynamic_timing: assembledModeAnalysis.best_window || assembledModeAnalysis.near_term_trend || raw.dayun_liunian?.turning_point || raw.dayun_liunian?.liuyue_trigger || ''
    },
    advice: {
      ...advice,
