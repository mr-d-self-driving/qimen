...
function buildModeAnalysisSchema(mode, secondaryMode) {
  ...
  if (mode === 'profile_driven') {
    ...
    "structural_reading": "120-200字，说明命局结构中与问题直接相关的十神强弱格局和先天倾向，点出哪条线突出、哪条线受限",
    "dayun_reading": "60-100字，当前大运对这个问题域的战略意义（印运/财运/食伤运各代表什么策略期）",
    "strategic_recommendation": "80-120字，综合命局结构和大运节奏给出主线策略建议，要有明确优先排序",
    "timeline": [
      { "period": "当前大运或年份区间（如 壬戌运2022-2031）", "strategy": "该阶段最适合的策略方向，一句话" },
      { "period": "下一个关键节点", "strategy": "建议策略" }
    ]
  }`;
  }
  return '{}';
...
/**
 * 格式化四柱+大运序列明细块，供 profile_driven 模式使用。
 * 只输出干支和藏干，不预计算十神（LLM 自行推导）。
 */
function formatSizhuDetailBlock(profile = {}) {
  ...
  if (dayunList.length > 0) {
    lines.push('');
    lines.push('【大运序列（前8步）】');
    const shown = dayunList.slice(0, 8);
    for (const d of shown) {
      const age = d.start_age != null ? `${d.start_age}岁` : '';
      const shen = d.shi_shen ? `（${d.shi_shen}）` : '';
      lines.push(`  ${age}：${d.gan || ''}${d.zhi || ''}${shen}`);
    }
  }
...
【分析要求】
本题是开放型战略问题，系统判断无法精确锁定单一目标元素，因此提供完整命盘交由你自主推演。请：
1. 识别与用户问题最相关的十神结构（如：主业→官印体系；副业/创业→食伤财体系；人际→六亲宫位），明确哪条线在原局中更突出；
2. 结合日主旺衰，判断先天"承载能力"及派生策略含义（如财多身弱→喜比劫→合伙优于单扛）；
3. 分析当前大运阶段的战略节奏意义（印运/食伤运/财运分别代表什么策略重心）；
4. 针对未来2-3个大运节点给出方向性建议，帮助用户理解何时可以加速、何时应该稳固；
5. 给出有明确优先排序的结论，不要模棱两可。
...
function buildBaziQuestionPrompt({ profile, question, route = {} }) {
  ...
  // ── 路由置信度低：绕过 pipeline，由 LLM 自主接管分析框架 ──
  if (semanticRoute.confidence === 'low') {
