...
score 始终为 null，不要给数字分数。
{
  ...
  "verdict": "一句话定论（40-80字）：开门见山回答用户的问题，包含方向判断+核心理由+时机节奏。不要写成标题，要写成完整的判断句。",
  "key_signals": [
    {
      "title": "信号标题（如：正财透月干，异性缘极旺但选择困难）",
      "reading": "60-120字，解释这个信号是什么→对用户问题意味着什么。必须引用具体干支，用生活语言解释命理含义，不得只罗列术语。"
    }
  ],
    ...
    "segments": [
      {
        ...
        "dayun_shishen": "大运十神",
        "strategy": "该阶段对本问题的核心策略方向，一句话",
        "key_liunians": [
          {
            "year": 2026,
            "gz": "丙午",
            "shishen": "食神",
            "note": "该年对问题域的具体意义——必须说清好在哪/坏在哪、怎么用，不得只写'利好'或'宜把握'"
          }
        ]
      }
    ]
  ...
  "action_guide": {
    "do": ["具体该做什么（可执行层面，不要写'把握机会'这类空话）"],
    "avoid": ["具体该避什么（落到行为层面）"],
    "hidden_insight": "一句话：命局结构暗示的、命主大概率没意识到的盲点或隐藏代价"
  }
}`;
}
...
function buildReadingsSchema(mode, secondaryMode) {
  if (mode === 'status') {
    return `{
    "psychological_mirror": "40-70字，用第一人称口吻描述命主此刻最可能有的内心驱动，禁用命理术语，改用职场/生活语言（如：坐不住、觉得投入产出不划算、不想被管、想拿作品说话）",
    "movement_nature": {
      "type": "主动换|被动换|内部变动|暂无明显变动",
      "quality": "顺势|逆势|中性",
      "evidence": "一句话命局依据"
    },
    "dayun_reading": "80-140字，说明当前大运如何建场、承接或压制本命题，必须引用系统断语",
    "liunian_reading": "80-140字，说明当前流年如何触发、放大、缓解或反向催动本命题，必须引用系统断语",
    "target_state_reading": "80-140字，说明目标十神/宫位当前状态和变化；若为事业/工作类，必须用具体职场语言描述（如：对上级容忍度、职责边界感受、岗位稳定性体感、投入产出感知），不得只写星曜受克",
    "outcome_projection": {
      "if_happens": "若变动发生，新局面的客观特征——具体到环境类型/职责变化/收入结构变化，不得笼统说'会有新机会'",
      "satisfaction_forecast": "高|中|低 + 一句核心原因——必须说清具体维度（钱、成长、自由度、人际）",
      "hidden_catch": "一句话：命局结构暗示的隐藏代价或制约"
    }
  }`;
  }
  if (mode === 'timing') {
    ...
    "trigger_windows": [
      {
        ...
        "ganzhi": "戊申",
        "dayun_ganzhi": "",
        "verdict": "70-130字，说明该年为什么被筛选出来——必须引用具体的干支机制（冲/合/透干/引动），不得只写'信号较强'",
  ...
  if (mode === 'pattern') {
    ...
    "pattern_verdict": "120-220字，综合先天适配、容量、优势与风险",
    "structural_supports": [],
    "structural_risks": [],
    "real_world_expression": "60-100字，这套先天结构在日常工作/生活里的具体表现——用行为场景描述（如：开会时习惯主导议程、遇到模糊任务会自己定义边界、容易对细节失去耐心），不得写成十神翻译",
    "how_to_leverage": "60-100字，顺着这个禀赋具体该往哪个方向使劲——必须落到可执行层面（行业方向/岗位类型/合作模式/技能投资），不得只写'发挥优势'"${secondaryMode === 'status' ? `,
  ...
  if (mode === 'profile_driven') {
    return `{
    "framework_used": "说明用了哪套分析框架（如：官印体系+食伤财体系+大运节奏分析），一句话",
    "psychological_mirror": "40-80字，用第一人称口吻描述命主此刻内心最真实的状态和驱动，禁用命理术语，用日常情绪语言",
    "structural_reading": "120-200字，说明命局结构中与问题直接相关的十神强弱格局和先天倾向；若问A vs B则分别评估",
    "path_readings": [
      {
        "path": "路径名称（如：主业优先 / 副业优先 / 创业 / 打工）",
        "structural_fit": "一句话：这条路对应哪套十神，原局里这套星曜的强弱如何",
        "likely_experience": "80-120字，走这条路近1-3年最可能经历什么（工作环境、人际、收入、情绪），用生活语言",
        "satisfaction_prediction": "一句话：中期满意度预判（高/中/低）+核心原因",
        "peak_period": "最顺风的大运或流年区间",
        "risk": "一句话：最大的陷阱或错位风险"
      }
    ]
  }`;
...
${secondaryBlock}
【推演任务】
请判断命局在该领域的先天适配度，按以下要求输出：

verdict：一句话定论——先天容量判断 + 核心理由 + 适配方向。不要写”必然成功/必然失败”。

key_signals：拆出3-5条与该领域先天适配相关的结构信号，每条 title + reading。
1. 解释 targetSpec.fallback_level 对置信度的影响，非 subcategory 级匹配不得给 high confidence。

readings：
2. pattern_verdict 用120-220字整合先天适配、容量、优势与风险；
3. real_world_expression 必须用具体行为场景描述（如：开会时习惯主导节奏），不得写成十神白话翻译；
4. how_to_leverage 必须落到可执行层面（行业方向/岗位类型/合作模式/技能投资），不得只说”发挥优势”；
5. 若 secondary_mode=status，只在 current_status_note 中说明当前大运流年是否适合推进，不写成应期预测。

rhythm：按当前+下一步大运给出策略节奏。

action_guide：do 落到具体行为，hidden_insight 写先天结构里容易被忽视的盲点。
...
action_guide：do/avoid 落到与该人物的相处建议。
...
【推演任务】
请判断当前或指定阶段的问题领域气候，按以下要求输出：

verdict：开门见山回答用户问题，格式为 方向判断（高/中/低）+ 变动性质（主动/被动）+ 一句核心理由。示例：”今年换工作概率偏高，偏主动出击型——食伤旺带来强烈表达欲，伤官见官使当前职位稳定性下降。”

key_signals：拆出3-5条与问题直接相关的命局信号，每条有 title + reading。
1. 必须区分：原局底盘是否支持 → 当前大运是否建场 → 当前流年是否触发。
2. 引用系统断语中”引动有效/无效”、”双重引动/单一引动”等标签，不要回显数值字段。
3. 若 liunian_impact.zhi_relations 中存在 type=”三合” 且 is_transformed=true，必须写成独立信号（格式：”流年×与原局××合成×三合×局”），这是最重要的结构性信号。
4. 若存在”六合”且 is_transformed=true，判断 transformed_element 是喜用→正面信号；是忌仇→负面信号。不得因为是”合”就默认正面。

readings：
5. psychological_mirror 用”你现在……”或”内心里……”开头，用职场/生活情绪描述，不出现命理术语。
6. movement_nature.type 区分”主动换”（食伤大旺顺势驱动）和”被动换”（冲克官宫 + 逆势），不得默认写主动换。
7. dayun_reading / liunian_reading / target_state_reading 按系统断语展开，不新增字段。
8. outcome_projection 是”如果变动发生后的后续故事”：if_happens 写新环境客观特征，satisfaction_forecast 写具体维度满意度，hidden_catch 写隐藏代价。三个字段都要有实质内容，禁止空话。
9. 若系统断语中 liunian_impact.mechanisms 存在 type=”填实三合”，必须在 key_signals 中写”流年补完三合成局”。

rhythm：按大运分段给出当前+下一步大运的策略节奏，每段点出2-3个关键流年。

action_guide：do/avoid 落到具体行为层面，hidden_insight 写命主未意识到的盲点。
...
【推演任务】
请判断该问题领域的应期窗口，按以下要求输出：

verdict：一句话定论——最优窗口年份 + 核心理由 + 原局底盘是否支持。

key_signals：拆出3-5条与应期相关的结构信号（原局底盘、大运建场、候选年份触发机制）。

readings：
1. trigger_windows 只能来自候选年份断语包，由你负责筛选；未选中的不展示给用户。
2. best_window、avoid_window 只引用已筛选窗口或候选年份，不新增未扫描年份。
3. 每个窗口的 verdict 和 mechanisms_text 用自然语言叙述，不回显数值字段。
4. 对 estimated/fallback_current 的大运定位降低置信。
5. 若候选年份为空，降级说明资料不足。

rhythm：按大运分段说明每段对该问题的应期倾向。

action_guide：do/avoid 落到具体行为，hidden_insight 写容易被忽视的时机盲点。
...
function formatSizhuDetailBlock(profile = {}) {
  ...
  const liunianList = matrix.liunian_list || [];
 ...
 */
function buildProfileDrivenPrompt({ profile, question, route }) {
  const ctx = extractBaziQuestionContext(profile);
...
branch: bazi
category/subcategory: ${route.category || 'general'} / ${route.subcategory || ''}
analysis_mode: profile_driven（系统未做目标元素预定位，由 LLM 自主确定分析框架）
...
【推演任务】
本题是开放型战略问题，系统提供完整命盘（含未来50年流年大运）交由你自主推演，按以下要求输出：

verdict：开门见山回答用户问题（40-80字）——方向判断 + 核心理由 + 节奏建议。若是二选一，给出"当下选A，X年后看B"。不模棱两可。

key_signals：拆出3-5条与问题直接相关的命局信号，每条 title + reading（60-120字）。
- 识别与问题相关的十神线（如主业→官印；副业→食伤财），逐条评估强弱。
- 必须引用具体干支和藏干证据，不得只写"官星较旺"。
- 结合日主旺衰给出承载力判断。

readings：
- psychological_mirror：第一人称口吻，日常语言（禁用命理术语），40-80字。
- structural_reading：120-200字，与问题相关的十神线强弱评估；A vs B 则分别评估。
- path_readings：每条路径展开——structural_fit（引用干支）、likely_experience（生活场景描述80-120字，不写十神翻译）、satisfaction_prediction、peak_period、risk。

rhythm：按大运分组给策略节奏；每段点出2-4个关键流年。
- key_liunians.note 必须落到对问题域的具体意义（如"食伤旺利内容产品上线变现"），不只写"利好"或"宜把握"。

action_guide：do/avoid 具体可执行，hidden_insight 写盲点。

质量约束：
- 禁止"视情况而定""整体尚可""宜审慎""把握机会"等空话。
- 每个字段都必须有实质信息量。
...
function buildBaziQuestionPrompt({ profile, question, route = {} }) {
  ...
  // ── llm_derived 显式路由优先（不受 confidence 影响） ──
  ...
  // ── profile_driven：路由器主动判定为开放战略型问题，绕过 pipeline ──
  if (semanticRoute.analysis_mode === 'profile_driven') {
    return { prompt: buildProfileDrivenPrompt({ profile, question, route: semanticRoute }), pipelineResult: null };
  }
...
function normalizeBaziQuestionOutput(raw = {}, { question = '', route = {}, pipelineResult = null } = {}) {
  ...
  return {
    ...
    analysis: {
      ...
      pattern: assembledReadings.pattern_verdict || assembledReadings.verdict || foundation.base_state || raw.ming_ju_analysis?.ge_ju || '',
      god_help: Array.isArray(foundation.supports) ? foundation.supports.join('；') : raw.ming_ju_analysis?.shensha_note || '',
      dynamic_timing: assembledReadings.best_window || raw.dayun_liunian?.turning_point || raw.dayun_liunian?.liuyue_trigger || ''
    },
    advice: {
      ...legacyAdvice,
      strategy: actionGuide.do || (Array.isArray(legacyAdvice.strategy) ? legacyAdvice.strategy : []),
      risk: actionGuide.hidden_insight || legacyAdvice.risk || '',
      avoid: actionGuide.avoid || (Array.isArray(legacyAdvice.avoid) ? legacyAdvice.avoid : []),
      lucky_tips: {
