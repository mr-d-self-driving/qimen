...
function buildModeAnalysisSchema(mode, secondaryMode) {
  if (mode === 'status') {
    ...
    "outcome_projection": {
      "if_happens": "若变动发生，新局面的客观特征——具体到环境类型（大公司/小团队/自由职业）、职责变化方向（管理/专业/横向）、收入结构变化（稳薪/绩效占比/不确定期长度），不得笼统说'会有新机会'",
      "satisfaction_forecast": "高|中|低 + 一句核心原因——必须说清楚满意/不满意的具体维度（钱、成长、自由度、人际），不得只写'整体尚可'",
      "hidden_catch": "一句话：命局结构里暗示的、命主当下大概率没意识到的隐藏代价或制约（如：看似升职实则管理消耗精力、看似自由实则收入断档期比预期长）"
    }
  }`;
  }
  if (mode === 'timing') {
  ...
  if (mode === 'profile_driven') {
    ...
    "psychological_mirror": "40-80字，用第一人称口吻描述命主此刻内心最真实的状态和驱动——是坐不住、还是焦虑中稳固、还是想突破但顾虑重重？禁用命理术语，用日常情绪语言",
    "structural_reading": "120-200字，说明命局结构中与问题直接相关的十神强弱格局和先天倾向，点出哪条线突出、哪条线受限；若问的是A vs B，则分别对A、B对应的十神线做评估",
    "path_readings": [
      {
        "path": "路径名称（如：主业优先 / 副业优先 / 创业 / 打工）",
        "structural_fit": "一句话：这条路对应哪套十神，原局里这套星曜的强弱如何",
        "likely_experience": "80-120字，如果走这条路，命主在近1-3年内最可能经历什么——工作环境、人际、精力状态、收入波动、情绪感受，用具体生活语言描述",
        "satisfaction_prediction": "一句话：这条路的中期满意度预判（高/中/低）+核心原因",
        "peak_period": "这条路最顺风的大运或流年区间",
        "risk": "一句话：走这条路最大的陷阱或错位风险"
      }
    ],
    "strategic_recommendation": "80-120字，综合命局结构和大运节奏给出主线策略建议，要有明确优先排序和时机判断",
    "timeline": [
      {
        "period": "大运区间（如 壬戌运 2022-2031）",
        "dayun_shishen": "大运十神",
        "strategy": "该大运的核心策略方向，一句话",
        "key_liunians": [
          { "year": 2026, "gz": "丙午", "shishen": "食神", "note": "该年对问题域的关键意义，一句话" }
        ]
      }
    ]
  }`;
...
5. 不要写”必然成功/必然失败”；
6. real_world_expression 必须用具体行为场景描述这套结构在日常中怎么表现（如：开会时习惯主导节奏、对模糊任务主动定义边界），不得写成十神名词的白话翻译；
7. how_to_leverage 必须落到可执行层面——给出具体的行业方向、岗位类型、合作模式或技能投资建议，不得只说”发挥优势””顺势而为”等空话。
...
5. 当前阶段适合推进到什么程度。
请把当前阶段判断集中写入 dayun_reading、liunian_reading、target_state_reading、psychological_mirror、movement_nature 五个字段，不要新增机会、压力、趋势等字段。
不要输出”成功率”。如不输出分数，summary.score 使用 null。
5. 若系统断语中 liunian_impact.mechanisms 存在 type=”填实三合”，必须在 positive_signals 中写”流年补完三合成局（×合×局）”，不得写成”缺位待补”。
6. psychological_mirror 必须用”你现在……”或”内心里……”开头，用具体的职场/生活情绪描述驱动，不出现任何命理术语。
7. summary.conclusion 格式：概率/方向判断（高/中/低）+ 变动性质（主动/被动）+ 一句核心理由，示例：”今年换工作概率偏高，偏主动出击型——食伤旺带来强烈表达欲，伤官见官使当前职位稳定性下降。”
8. movement_nature.type 区分”主动换”（食伤大旺顺势驱动）和”被动换”（冲克官宫 + 逆势），不得默认写主动换；quality 结合忌神方向判断顺逆。
9. 若 liunian_impact.zhi_relations 中存在 type=”三合” 且 is_transformed=true 的条目，无论是否涉及目标十神宫位，都必须写入 positive_signals（格式：”流年×与原局××合成×三合×局”）；这是今年最重要的结构性信号，不得遗漏。
10. 若 liunian_impact.zhi_relations 中存在含”六合”且 is_transformed=true 的条目，必须判断 transformed_element 是喜用五行还是忌仇五行：是喜用→写入 positive_signals；是忌仇→写入 negative_signals。不得因为是”合”就默认写成正向信号。
11. outcome_projection 是”如果变动发生后的后续故事”，必须基于大运流年走势推演：if_happens 写新环境的客观特征（落到行业/职责/收入结构），satisfaction_forecast 写具体维度的满意度（不得笼统），hidden_catch 写命局暗示的命主尚未意识到的隐藏代价。三个字段都要有实质内容，禁止用”视情况而定””整体尚可”等空话。
...
分析步骤（每一步都必须产出实质内容，不得笼统、不得重复前面已说的话）：

1. 【结构定位】先识别与问题直接相关的十神线（如主业→官印体系；副业→食伤财体系），逐条评估该线在原局中的强弱——必须引用具体的干支和藏干证据，不得只写"官星较旺"这类空话。结合日主旺衰给出承载力判断。
2. 【心理镜像】用第一人称口吻，用日常语言（禁用命理术语）描述命主此刻最真实的内心状态——焦虑什么、想要什么、卡在哪里。40-80字。
3. 【路径故事】对问题中涉及的每条路径分别展开：
   - 对应哪套十神，原局这套星曜的强弱如何（一句话，引用具体干支）
   - 如果走这条路，近1-3年命主最可能经历什么（工作环境类型、人际质量、收入结构变化、精力消耗、情绪体感——用具体职场/生活语言，不用命理术语，80-120字）
   - 中期满意度预判（高/中/低）+ 核心原因（一句话）
   - 这条路最顺风的大运或流年区间
   - 这条路最大的陷阱或错位风险（一句话）
4. 【时间轴】按大运分组，逐段给出策略节奏（一句话核心方向）；每个大运内点出2-4个关键流年——每个流年必须说清"好在哪里/坏在哪里、怎么用"，不得只写"利好"或"宜把握"。
5. 【策略结论】有明确优先排序和时机判断，80-120字。若是二选一，给出"当下阶段选A，X年后看B"的节奏建议，不模棱两可。

质量约束：
- 每个字段必须产出具体、有信息量的内容，禁止"视情况而定""整体尚可""宜审慎""把握机会"等空话套话。
- path_readings 中 likely_experience 要用生活场景描述（如：上班像在给别人打工还是自己有主导权、同事关系是合作还是竞争、钱是稳薪还是靠绩效浮动），不要写成十神翻译。
- timeline 中 key_liunians.note 必须落到对问题域的具体意义（如"食伤旺利内容产品上线变现"），不要只写"运势上升"。
- summary.conclusion 格式：方向判断 + 核心理由 + 节奏建议，一句话概括。
