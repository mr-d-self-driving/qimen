...
function formatSizhuDetailBlock(profile = {}) {
  ...
  if (dayunList.length > 0) {
    ...
    for (const d of dayunList) {
      ...
      // 找属于这个大运的流年（按 gan/zhi 匹配，或用 start_age 区间估算）
      // 由于 liunian_list 有 year 字段，通过 start_age 推算出生年 → 再定位流年范围
      const birthYear = futureLiunians.length > 0
        ? (Number(futureLiunians[0]?.year) - (matrix.current_dayun?.start_age || 0))
        : null;
...
branch: bazi
category/subcategory: ${route.category || 'general'} / ${route.subcategory || ''}
analysis_mode: profile_driven（路由置信度低，系统未做目标元素预定位，由 LLM 自主确定分析框架）
...
【分析要求】
本题是开放型战略问题，系统提供完整命盘（含未来50年流年大运）交由你自主推演。请把这个故事讲完整：

1. 【心理镜像】先说命主此刻内心的真实状态——用第一人称口吻，用日常语言（不用命理术语）描述此刻最真实的内心驱动或焦虑。
2. 【结构分析】识别与问题相关的十神线（如主业→官印；副业→食伤财），评估每条线在原局中的强弱；结合日主旺衰给出承载力判断（如财多身弱→合伙优于单扛）。
3. 【路径故事】对问题中涉及的每条路径（A/B或多选项），各自展开：
   - 对应哪套十神结构，原局是否支持
   - 如果走这条路，近1-3年命主最可能经历什么（工作环境、人际、收入、情绪体感，用具体生活语言）
   - 中期满意度预判 + 这条路的核心风险
   - 这条路最顺风的时机窗口
4. 【时间轴】按大运分组，逐段给出策略节奏；每个大运内点出2-4个关键流年及其对该问题的具体意义（不只是"好"或"不好"，要说清楚好在哪里、怎么用）。
5. 【结论】有明确优先排序，不模棱两可；若是二选一，给出明确的"当下阶段选A，X年后看B"的节奏建议。
