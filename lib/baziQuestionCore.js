        ...
        "key_liunians": [
          {
            ...
            "gz": "丙午",
            "shishen": "食神",
            "note": "该年对问题域的具体意义——说清好在哪/坏在哪/怎么用，不得只写'利好'或'宜把握'"
          }
        ]
      }
    ]
...
function buildReadingsSchema(mode, secondaryMode) {
  ...
  if (mode === 'status') {
    ...
    "base_foundation": {
      ...
      "signals": ["1-3条：原局底盘+目标用神状态中最关键的信号，标题≤20字+一句话说明"]
    },
    "target_state": [
      {
    ...
    "liunian_trigger": {
      ...
      "phenomena": [
        {
          "tag": "≤10字现象标签（引用引动机制清单，如：子午冲妻宫·有效）",
...
1. 引用【引动机制清单】中的机制标签（有效/潜伏），不回显数值字段。
...
readings.liunian_trigger.phenomena[]：从【引动机制清单】里选≤4条重要的”移动”（有效或高强度的），每条 tag(≤10字)+explain(≤40字，说明对本问题的含义）。

rhythm：当前+下一步大运策略节奏，每段点出2-3个关键流年（note 说清对本问题域的意义）。
...
rhythm：按大运分段说明应期倾向，key_liunians 须引用已注入的具体流年干支。
...
rhythm：按大运分组；key_liunians.note 必须落到问题域具体意义，引用已注入流年干支，不只写"利好/宜把握"。
...
/**
 * 引动强度 5 档（陆致极理论，§9.12）
 * 优先级：首重虚实(trigger_vigor) → 次看岁运共振(is_major_window) → 末看机制威力(effective_strength)
 * @param {{ v: number, major: boolean, s: number }} p
 */
...
function normalizeBaziQuestionOutput(raw = {}, { question = '', route = {}, pipelineResult = null } = {}) {
  ...
  if (pipelineResult && meta.analysis_mode === 'timing' && pipelineResult.timingCandidates?.length) {
    ...
    const assembledWindows = llmWindows.map((llmWindow) => {
      ...
      // strength 5档：后端权威（陆致极理论，§9.12），覆盖 LLM 自填
      const ppl = candidate._pipeline || {};
      const topV = Math.max(ppl.trigger_vigor?.dayun ?? 0, ppl.trigger_vigor?.liunian ?? 0);
      const topS = ppl.activation_strength ?? 0;
      const isMaj = candidate.is_major_window || false;
      const strength = deriveStrengthTier({ v: topV, major: isMaj, s: topS });
