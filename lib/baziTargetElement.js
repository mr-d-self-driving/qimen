  ...
  conception: {
    gender_variants: {
      ...
      male: {
        ...
        analysis_question:
          '男命添丁/得子应期：以父方命盘观察子女缘启动窗口，不把命主表述为受孕主体。时柱（子女宫）旺相且大运流年合动/透出子女星时，为配偶怀孕、家庭添丁或子女缘被引动的应期。同时检查有无命局层面少子/迟子特征（印重克食伤、五行极偏）。',
      },
      female: {
 ...
 */
// UI/DB 存储的 category 简称 → CATEGORY_FALLBACK 内部 key 映射
const CATEGORY_ALIAS = {
  career:   'career_business',
  wealth:   'finance_wealth',
  love:     'relationship',
  health:   'health',
  misc:     null,   // 杂事：无特定映射，走通用回退
  general:  null,
}
...
function resolveTargetElement({ category, subcategory, gender } = {}) {
  ...
  if (subcategory && SUBCATEGORY_MAP[subcategory]) {
    ...
    return { ...spec, category, subcategory, fallback_level: 'subcategory' };
  }

  // 2. 尝试 category 级回退（支持 UI 简称别名）
  const resolvedCategory = (category && CATEGORY_ALIAS.hasOwnProperty(category))
    ? CATEGORY_ALIAS[category]
    : category
  if (resolvedCategory && CATEGORY_FALLBACK[resolvedCategory]) {
    const spec = applyGenderAdjustment(CATEGORY_FALLBACK[resolvedCategory], gender);
    // relationship category 回退时补充性别对应的妻/夫星
    if (resolvedCategory === 'relationship') {
      const shishen = gender === 'female' ? ['正官', '七杀'] : ['正财', '偏财'];
      return { ...spec, primary_shishen: shishen, category, subcategory: null, fallback_level: 'category' };
    }
    if (resolvedCategory === 'pregnancy_birth') {
      const shishen = gender === 'female' ? ['食神', '伤官'] : ['正官', '七杀', '食神', '伤官'];
      return { ...spec, primary_shishen: shishen, category, subcategory: null, fallback_level: 'category' };
    }
    return { ...spec, category, subcategory: null, fallback_level: 'category' };
  }

  // 3. 通用回退：原局喜忌 + 格局
  return { ...GENERAL_FALLBACK, category: category ?? null, subcategory: subcategory ?? null };
}
