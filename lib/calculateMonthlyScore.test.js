...
const flowMonthRange = (monthGz, start, end) => ({
  ...flowMonth(monthGz),
  start_date: start,
  end_date: end,
});
...
test('monthly layer weights use 60/10/30 distribution', () => {
  ...
  assert.ok(result.layer2_score >= -10 && result.layer2_score <= 10);
  assert.ok(result.layer3_score >= -15 && result.layer3_score <= 15);
  assert.ok(result.layer2_score > 0);
});

test('monthly layer2 ignores plain high averages from ganzhi rotation', () => {
  const result = calculateMonthlyScore(
    baseProfile,
    flowMonthRange('丁酉', '2026-01-10', '2026-01-20'),
    () => ({ final_score: 82 }),
    neutralShiShen
  );

  assert.ok(result.avg_daily_score > 75);
  assert.ok(result.layer2_score <= 1, 'plain rotating high days should not dominate month score');
  assert.ok(result.score_hits.layers.some(layer => (
    layer.layer === 'layer2'
    && layer.hits.some(hit => hit.code === 'daily_rotation_baseline')
  )));
});

test('monthly layer2 unlocks stronger penalty for structural low-day clusters', () => {
  let calls = 0;
  const result = calculateMonthlyScore(
    baseProfile,
    flowMonthRange('丁酉', '2026-01-10', '2026-01-20'),
    () => {
      calls += 1;
      if (calls >= 3 && calls <= 8) return { final_score: 38, has_sanxing: true };
      return { final_score: 70 };
    },
    neutralShiShen
  );

  assert.ok(result.layer2_score <= -6, 'structural low-day clusters should unlock beyond the old -5 floor');
  assert.ok(result.score_hits.layers.some(layer => (
    layer.layer === 'layer2'
    && layer.hits.some(hit => hit.code === 'layer2_negative_gate_unlocked')
  )));
});
...
test('monthly tianyi is discounted by shensha auxiliary weighting', () => {
  const unfavorable = calculateMonthlyScore(
    {
      ...baseProfile,
      ri_zhu: '甲午',
      day_zhi: '午',
  ...
  const unfavorableHit = unfavorable.score_hits.layers[2].hits.find(hit => hit.code === 'tianyi_adjusted');

  assert.equal(unfavorable.shishen_signals.tianyi_active, true);
  assert.ok(unfavorableHit);
  assert.equal(unfavorableHit.delta_raw, 0);
  assert.match(unfavorableHit.debug, /base=6/);
  assert.match(unfavorableHit.debug, /tier=ji/);

  const favorable = calculateMonthlyScore(
    {
      ...baseProfile,
      ri_zhu: '甲午',
      day_zhi: '午',
  ...
  const favorableHit = favorable.score_hits.layers[2].hits.find(hit => hit.code === 'tianyi_adjusted');

  assert.ok(favorableHit);
  assert.equal(favorableHit.delta_raw, 6);
  assert.ok(favorable.layer3_score <= 15);
});
