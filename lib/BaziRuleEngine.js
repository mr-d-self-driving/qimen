...
const BaziRuleEngine = {
  ...
  calculateStrength(dayGan, gans, zhis) {
    ...
    const groundText = groundedBranches.length
      ? `通根得地以年/日/时三支计分：${groundedBranches.map(item => `${item.pillar}${item.zhi}为${item.phase}，计${formatScore(item.score)}分`).join('；')}；合计${formatScore(groundScore)}分。`
      : '年支、日支、时支皆无显著根气，通根得地合计0分。';
    const stemText = stemHelpBreakdown.length
      ? `天干帮扶方面：${stemHelpBreakdown.map(item => `${item.pillar}${item.gan}为${item.shen}，${item.hasRoot ? '透干有根' : '透干无根'}，计${item.score}分`).join('；')}；合计${stemHelpScore}分。`
      : '年干、月干、时干无比劫印枭透出，天干帮扶计0分。';
    const finalText = `综合得分为${formatScore(totalStrengthScore)}分，故基础判为${strongWeak === '专旺格' || strongWeak.includes('从格') ? '常规格局之外' : strongWeak}。`;
    ...
    if (!dayMasterHasRoot && !stemHasHelp && isDominant) {
      ...
      congCheckText = `从格校验：${dayMasterHasRoot ? '日主有根' : '日主真无根'}，${stemHasHelp ? '天干有帮身' : '天干无帮身'}，${dominantElement}占比约${Math.round(dominantRatio * 100)}%，不触发从格/专旺覆盖。`;
    }
