...
const BaziRuleEngine = {
  ...
  calculateStrength(dayGan, gans, zhis) {
    ...
    const monthMainGan = C.ZHI5_LIST[monthZhi]?.[0];
    const monthMainShen = monthMainGan ? C.SHISHEN[dayGan][monthMainGan] : null;
    const outputControlKilling = gans.some((gan, index) => {
      if (index === 2) return false;
      const shen = C.SHISHEN[dayGan][gan];
      if (shen !== '食' && shen !== '伤') return false;
      return gans.some((targetGan, targetIndex) => {
        if (targetIndex === 2) return false;
        const targetShen = C.SHISHEN[dayGan][targetGan];
        if (targetShen !== '官' && targetShen !== '杀') return false;
        return RESTRAINING_CYCLE[C.GAN5[targetGan]] === C.GAN5[gan];
      });
    });
    const doubleLuFoodControlsKilling =
      !!luZhi
      && zhis[2] === luZhi
      && zhis[3] === luZhi
      && (monthMainShen === '官' || monthMainShen === '杀')
      && outputControlKilling;
    let doubleLuAdjustment = 0;
    if (doubleLuFoodControlsKilling && preliminaryStrengthScore < 4.3) {
      doubleLuAdjustment = 4.3 - preliminaryStrengthScore;
      structureAdjustment += doubleLuAdjustment;
    }

    const strongPhaseCount = meStatus.filter(phase => STRONG_ROOT_PHASES.has(phase)).length;
    const dayHourStrongRootCount = [dayPhase, hourPhase].filter(phase => STRONG_ROOT_PHASES.has(phase)).length;
    const rootedVisibleSupportCount = supportDetail.support_stems.filter(item => item.rooted).length;
    const supportOrCompanion =
      supportDetail.score >= 1.1
      || rootedVisibleSupportCount >= 1
      || dayHourStrongRootCount >= 2
      || strongPhaseCount >= 3;
    const repeatedEffectiveRoots = rootDetail.roots.length >= 2 && rootDetail.score >= 1.4;
    const hardRootNetwork = strongPhaseCount >= 3 && rootDetail.score >= 2.5;
    const dayHourRootNetwork = dayHourStrongRootCount >= 1 && repeatedEffectiveRoots;
    const supportedRootNetwork = repeatedEffectiveRoots && supportOrCompanion;
    const metalWaterRatio = totalWuxingScore > 0 ? ((scores['金'] || 0) + (scores['水'] || 0)) / totalWuxingScore : 0;
    const fireRatio = totalWuxingScore > 0 ? (scores['火'] || 0) / totalWuxingScore : 0;
    const woodRatio = totalWuxingScore > 0 ? (scores['木'] || 0) / totalWuxingScore : 0;
    const wetEarthCount = zhis.filter(zhi => zhi === '辰' || zhi === '丑').length;
    const springWoodBranchCount = zhis.filter(zhi => zhi === '寅' || zhi === '卯').length;
    const visibleOutputStemCount = gans.filter((gan, index) => {
      if (index === 2) return false;
      const shen = C.SHISHEN[dayGan][gan];
      return shen === '食' || shen === '伤';
    }).length;
    const visibleOfficerKillingStemCount = gans.filter((gan, index) => {
      if (index === 2) return false;
      const shen = C.SHISHEN[dayGan][gan];
      return shen === '官' || shen === '杀';
    }).length;
    const springEarthPressure =
      ['戊', '己'].includes(dayGan)
      && ['寅', '卯'].includes(monthZhi)
      && (woodRatio >= 0.25 || monthMainShen === '官' || monthMainShen === '杀');
    const killingSealWithoutFoodControl =
      (monthMainShen === '官' || monthMainShen === '杀')
      && !outputControlKilling;
    const summerWoodNeedsSeal =
      ['甲', '乙'].includes(dayGan)
      && ['巳', '午', '未'].includes(monthZhi)
      && dayHourStrongRootCount === 0;
    const coldWetEarthOverleak =
      ['戊', '己'].includes(dayGan)
      && (['丑', '辰'].includes(monthZhi) || wetEarthCount >= 2)
      && (metalWaterRatio >= 0.38 || (visibleOutputStemCount >= 2 && metalWaterRatio >= 0.3))
      && fireRatio < 0.18;
    const winterEarthPressure =
      ['戊', '己'].includes(dayGan)
      && ['亥', '子', '丑'].includes(monthZhi)
      && metalWaterRatio >= 0.35;
    let coldWetEarthOverleakAdjustment = 0;
    const beforeColdWetScore = clamp(seasonDetail.score + rootDetail.score + supportDetail.score + structureAdjustment, 0, 10);
    if (
      coldWetEarthOverleak
      && (wetEarthCount >= 2 || visibleOutputStemCount >= 2 || dominantElement === '金')
      && beforeColdWetScore > 4.15
    ) {
      coldWetEarthOverleakAdjustment = 4.15 - beforeColdWetScore;
      structureAdjustment += coldWetEarthOverleakAdjustment;
    }

    let springEarthKillingPressureAdjustment = 0;
    const springEarthKillingPressure =
      springEarthPressure
      && (springWoodBranchCount >= 2 || visibleOfficerKillingStemCount >= 1 || woodRatio >= 0.32)
      && !(fireRatio >= 0.38 && supportDetail.score >= 2.2 && rootDetail.score >= 2.8);
    const beforeSpringEarthScore = clamp(seasonDetail.score + rootDetail.score + supportDetail.score + structureAdjustment, 0, 10);
    if (springEarthKillingPressure && beforeSpringEarthScore > 4.15 && beforeSpringEarthScore <= 5.55) {
      springEarthKillingPressureAdjustment = 4.15 - beforeSpringEarthScore;
      structureAdjustment += springEarthKillingPressureAdjustment;
    }

    let earlySpringFireVoidAdjustment = 0;
    const earlySpringFireVoid =
      ['丙', '丁'].includes(dayGan)
      && monthZhi === '寅'
      && metalWaterRatio >= 0.36
      && fireRatio < 0.22
      && rootDetail.score <= 1.6
      && supportDetail.score <= 1.4;
    const beforeEarlySpringFireScore = clamp(seasonDetail.score + rootDetail.score + supportDetail.score + structureAdjustment, 0, 10);
    if (earlySpringFireVoid && beforeEarlySpringFireScore > 4.15 && beforeEarlySpringFireScore <= 5.55) {
      earlySpringFireVoidAdjustment = 4.15 - beforeEarlySpringFireScore;
      structureAdjustment += earlySpringFireVoidAdjustment;
    }

    const rootNetworkBaseScore = clamp(seasonDetail.score + rootDetail.score + supportDetail.score + structureAdjustment, 0, 10);
    let rootNetworkAdjustment = 0;
    if (!springEarthPressure && !killingSealWithoutFoodControl && !summerWoodNeedsSeal && !coldWetEarthOverleak && !winterEarthPressure && !earlySpringFireVoid && rootNetworkBaseScore >= 2.8 && rootNetworkBaseScore < 5.65) {
      if (doubleLuFoodControlsKilling && dayHourStrongRootCount >= 2) {
        rootNetworkAdjustment = Math.max(rootNetworkAdjustment, 5.6 - rootNetworkBaseScore);
      }
      if (hardRootNetwork) {
        rootNetworkAdjustment = Math.max(rootNetworkAdjustment, 5.65 - rootNetworkBaseScore);
      }
      if (dayHourRootNetwork && supportOrCompanion) {
        rootNetworkAdjustment = Math.max(rootNetworkAdjustment, 5.65 - rootNetworkBaseScore);
      }
      if (supportedRootNetwork && supportDetail.score >= 1.1 && rootNetworkBaseScore >= 3.8) {
        rootNetworkAdjustment = Math.max(rootNetworkAdjustment, 5.65 - rootNetworkBaseScore);
      }
    }
    rootNetworkAdjustment = clamp(rootNetworkAdjustment, 0, 1.8);
    if (rootNetworkAdjustment > 0) {
      structureAdjustment += rootNetworkAdjustment;
    }
    ...
    if (doubleLuAdjustment) structureLabel += '，两重禄根且食伤制官杀，日主不按弱论';
    if (rootNetworkAdjustment) structureLabel += '，有效根网/禄刃根成立，日主不按弱论';
    if (coldWetEarthOverleakAdjustment) structureLabel += '，寒湿土金水过泄，辰丑根不作纯帮身';
    if (springEarthKillingPressureAdjustment) structureLabel += '，春木官杀压土，印根未足以转强';
    if (earlySpringFireVoidAdjustment) structureLabel += '，初春火虚木嫩，金水压力未化尽';
    if (!dayMasterHasRoot && !stemHasHelp && isDominant) {
      ...
      if (doubleLuAdjustment) congCheckText += ' 日时两重禄根，且食伤透出制官杀，日主不按弱论。';
      if (rootNetworkAdjustment) congCheckText += ' 有效根网/禄刃根成立，日主不按弱论。';
      if (coldWetEarthOverleakAdjustment) congCheckText += ' 寒湿土金水过泄，辰丑湿根以蓄泄为主，不按纯帮身论。';
      if (springEarthKillingPressureAdjustment) congCheckText += ' 春木官杀压土，虽有印根但尚未转成身强。';
      if (earlySpringFireVoidAdjustment) congCheckText += ' 初春火虚木嫩，金水压力偏重，不按印根成势论。';
    }
    ...
    const structureException = {
      ...
      cold_wet_earth_overleak_adjustment: coldWetEarthOverleakAdjustment,
      spring_earth_killing_pressure_adjustment: springEarthKillingPressureAdjustment,
      early_spring_fire_void_adjustment: earlySpringFireVoidAdjustment,
    ...
    const strengthDetail = {
      ...
      metrics: {
        ...
        double_lu_food_controls_killing_adjustment: doubleLuAdjustment,
        root_network_adjustment: rootNetworkAdjustment,
        cold_wet_earth_overleak_adjustment: coldWetEarthOverleakAdjustment,
        spring_earth_killing_pressure_adjustment: springEarthKillingPressureAdjustment,
        early_spring_fire_void_adjustment: earlySpringFireVoidAdjustment,
      },
   ...
   * 维度 4: 扶抑 (Fuyi)
   */
  getFavorableUnfavorable(dayGan, monthZhi, geJu, strengthResult, zhis, gans, geJuInfoRecord = null, imageAnalysis = null, chengGeDetail = null, classicalPattern = null) {
    const SHISHEN_NAMES = [
      "比肩", "劫财",   // 比劫类
      "食神", "伤官",   // 食伤类
      "正财", "偏财",   // 财类
    ...
    const classicalTags = new Set(classicalPattern?.methodTags || []);
    const classicalAliases = new Set(classicalPattern?.aliases || []);
    const hasClassicalSignal = signal =>
      classicalPattern?.name === signal || classicalTags.has(signal) || classicalAliases.has(signal);
    const classicalEffects = { yongShens: [], unfavorableShens: [], reasons: [] };

    const getClassicalSpecialPattern = () => {
      if (!hasClassicalSignal('魁罡格') || !hasClassicalSignal('财官印取用')) return null;
      const yongPriority = sortShensByTransparency(['正印', '偏印', '正财', '偏财', '正官', '七杀']);
      return {
        override_scope: 'classical_yongshen',
        type: '魁罡',
        yongPriority,
        favorable: ['正印', '偏印', '正财', '偏财', '正官', '七杀', '食神', '伤官'],
        unfavorable: [],
        scoreMode: 'ranked'
      };
    };
    ...
    const getSpecialPattern = () => {
      ...
      if (candidate.category === 'SINGLE_IMAGE') {
        ...
        if (completeWoodFrame) {
          const yongPriority = sortShensByTransparency(['比肩', '劫财', ...frameSealShens]);
          return {
            ...overrideMeta,
            type: '专旺',
            yongPriority,
            favorable: [...new Set(['比肩', '劫财', ...frameSealShens, '食神', '伤官'])],
            unfavorable: ['正官', '七杀', '正财', '偏财'],
            scoreMode: 'single_image_ranked'
          };
        }
      ...
      if (candidate.subtype === '从官杀格' || candidate.subtype === '从杀格') {
        const officerKillingPriority = sortShensByTransparency(['正官', '七杀']);
        const wealthPriority = sortShensByTransparency(['正财', '偏财']);
        return {
          ...overrideMeta,
          type: candidate.subtype === '从杀格' ? '从杀' : '从官杀',
          favorable: ['正官', '七杀', '正财', '偏财'],
          unfavorable: ['正印', '偏印', '比肩', '劫财', '食神', '伤官'],
          yongPriority: [...officerKillingPriority, ...wealthPriority],
          scoreMode: 'ranked'
        };
      }
    ...
    const specialPattern = getClassicalSpecialPattern() || getSpecialPattern();
    if (specialPattern) {
    ...
    const applyClassicalPatternEffects = () => {
      const promoteClassicalYong = (shen, margin = 5) => {
        const topScore = Math.max(...Object.values(scores));
        const target = topScore + margin;
        const delta = Math.max(0, target - scores[shen]);
        if (delta > 0) {
          scores[shen] += delta;
          breakdown[shen].conflict += delta;
        }
        if (!classicalEffects.yongShens.includes(shen)) classicalEffects.yongShens.push(shen);
      };
      const demoteClassicalUnfavorable = (shens, target = -35) => {
        ...
        classicalEffects.unfavorableShens.push(...shens);
      };

      if (hasClassicalSignal('印绶天德') && hasClassicalSignal('印绶')) {
        promoteClassicalYong('正印');
        if (scores['偏印'] < 20) {
          const assistantDelta = 20 - scores['偏印'];
          scores['偏印'] += assistantDelta;
          breakdown['偏印'].conflict += assistantDelta;
        }
        classicalEffects.reasons.push('印绶天德：印绶与天德同宫，印星取用优先于普通寒月泄秀调候。');
      }

      if (hasClassicalSignal('日德格') && hasClassicalSignal('犯忌')) {
        const targets = ['正财', '偏财', '正官', '七杀', '食神'];
        demoteClassicalUnfavorable(targets, -45);
        classicalEffects.reasons.push('日德格犯忌：见庚辰、财星官旺乡及刑冲，财官与庚辰食神证据入忌。');
      }

      if (hasClassicalSignal('伤官吐秀') && hasClassicalSignal('卯木为用')) {
        promoteClassicalYong('伤官');
        if (hasClassicalSignal('酉冲破用')) {
          demoteClassicalUnfavorable(['正印', '偏印'], -25);
        }
        classicalEffects.reasons.push('伤官吐秀：两辰拱水、两庚印透而身转强，取卯木伤官吐秀为用；酉金冲破卯木则印金入忌。');
      }

      if (hasClassicalSignal('财旺生官') && hasClassicalSignal('去火存金') && hasClassicalSignal('体用不伤')) {
        promoteClassicalYong('偏印');
        demoteClassicalUnfavorable(['正财', '偏财', '正官', '七杀'], -35);
        classicalEffects.reasons.push('财旺生官去火存金：癸亥时日主之气已贯，巳亥遥冲去火存金，取辛金偏印为用，火财土官杀不再越位。');
      }
    };
    applyClassicalPatternEffects();
    ...
    if (chengGeEffects.assistantShens.length && chengGeEffects.yongShens.length && !classicalEffects.yongShens.length && !yinRescue && !yangRenDriveSha) {
    ...
    const protectRootNetworkBearingShens = () => {
      const metrics = strengthResult.strengthDetail?.metrics || {};
      const hasRootNetworkCorrection =
        Number(metrics.root_network_adjustment || 0) > 0
        || Number(metrics.double_lu_food_controls_killing_adjustment || 0) > 0
        || Number(metrics.winter_water_network_adjustment || 0) > 0;
      if (!hasRootNetworkCorrection) return;
      for (const shen of ['比肩', '劫财']) {
        if (scores[shen] >= 0) continue;
        const delta = -scores[shen];
        scores[shen] = 0;
        breakdown[shen].conflict += delta;
      }
    };
    protectRootNetworkBearingShens();
    ...
    if (classicalEffects.reasons.length) {
      decisionChain.push({
        layer: 'L2 古法取用',
        reason: classicalEffects.reasons.join('；'),
        override: 'source-backed classical pattern'
      });
    }
