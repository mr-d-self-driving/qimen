...
const BaziRuleEngine = {
   ...
   * 维度 4: 扶抑 (Fuyi)
   */
  getFavorableUnfavorable(dayGan, monthZhi, geJu, strengthResult, zhis, gans, geJuInfoRecord = null) {
    const SHISHEN_NAMES = [
      "比肩", "劫财",   // 比劫类
      "食神", "伤官",   // 食伤类
      "正财", "偏财",   // 财类
    ...
    SHISHEN_NAMES.forEach(shen => {
      scores[shen] = 0;
      breakdown[shen] = { tiaohou: 0, bingyao: 0, tongguan: 0, fuyi: 0, geju: 0, cong: 0, conflict: 0 };
    });
    ...
    if (specialPattern) {
      ...
      const spDecisionChain = [{ layer: 'L0 特殊气势', reason: `识别为${specialPattern.type}，顺其成势，跳过常规四维。`, override: '覆盖全部下层' }];
      return {
        wuxing,
