...
function buildDomainViewPromptSection(rule) {
  if (!rule?.domainView) {
    return `
**【M2 领域用神卡片规则】**
当前问题域暂无专属领域 axes。请按通用结构输出 qimen_report.m2_basis.yongshen_cards：
- subject：问测人 = 日干
- target：目标事态/物品/对方 = 时干或领域主用神
- environment：关键环境因素 = 值使门、值符或核心门星神
...
不要额外输出 domain_view。`;
  ...
  return `
**【M2 领域用神卡片规则】**
后端已按当前问题域定义好用神拆分 axes。你不要自行发明展示单元，也不要把它压扁成通用 subject/target/environment 三卡。

请将以下 axes 逐项写入 qimen_report.m2_basis.yongshen_cards，每个 axis 生成一张卡：
${axes}

每张卡必须包含：
- key：沿用上方 axis.key
- label：沿用上方 axis.label
- symbol：沿用上方 axis.symbol，可补充实际盘面符号，如"日干 戊"、"时干 癸"
- tone：positive、mixed、warning 三选一
- verdict：一句话说明该单元的现实状态
- evidence：必须对应具体宫位/门星神干/空亡马星/有名格/生克关系，不要只写抽象判断

原 domain_view.process/timing/decision 不再作为独立展示字段输出：
- process（${rule.domainView.process.label}，参考 ${rule.domainView.process.symbols.join('、')}）的含义并入 m3_inference.environment_state.reading 或 interaction_decision.reading。
- timing（${rule.domainView.timing.label}）并入 m4_guidance.timing_behavior。
- decision（${rule.domainView.decision.label}）并入 m1_conclusion.actions 或 m3_inference.interaction_decision。

不要额外输出 domain_view；旧记录由前端 fallback 读取。`;
}
