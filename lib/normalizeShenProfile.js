...
function expandShiShenItem(item) {
  return SHISHEN_ALIASES[item] || [item];
}

function shiShenItemsConflict(a, b) {
  if (a === b) return true;
  const aSet = new Set(expandShiShenItem(a));
  return expandShiShenItem(b).some(item => aSet.has(item));
}

function removeLowerTierConflicts(tiers) {
  const occupied = [];
  return tiers.map(list => {
    const kept = [];
    for (const item of list) {
      const conflictsWithHigher = occupied.some(existing => shiShenItemsConflict(existing, item));
      const duplicatesCurrent = kept.some(existing => shiShenItemsConflict(existing, item));
      if (!conflictsWithHigher && !duplicatesCurrent) {
        kept.push(item);
        occupied.push(item);
      }
    }
...
function normalizeShenProfile(profile) {
  ...
  if (hasNew) {
    const [yong, xi, ji, chou] = removeLowerTierConflicts([
      normalizeList(profile.yong_shen),
      normalizeList(profile.xi_shen),
      normalizeList(profile.ji_shen),
      normalizeList(profile.chou_shen),
    ]);
    return {
      yong,
      xi,
  ...
  // 旧格式降级
  const [yong, xi, ji, chou] = removeLowerTierConflicts([
    normalizeList(profile.favorable_elements),
    [],
    normalizeList(profile.unfavorable_elements),
    [],
  ]);
